...
static void kconfig_draw_contents(kc_menu *menu, grs_canvas *canvas)
{
...
void kconfig_draw(kc_menu *menu)
{
	...
	kconfig_draw_contents(menu, window_get_canvas(menu->wind));
}
...
void kconfig_fill_joy_settings(const int *indices, const int *values, int count, ubyte *out)
{
	kconfig_android_fill_joy_settings(
		kconfig_android_get_layout(KCONFIG_ANDROID_D2), indices, values,
...
void kconfig_get_default_settings(ubyte *kb_out, ubyte *joy_out, ubyte *mouse_out)
{
	...
	kconfig_android_apply_default_overrides(
		kconfig_android_get_layout(KCONFIG_ANDROID_D2), joy_out, MAX_CONTROLS);
}
