...
#ifdef __ANDROID__
static int local_death_pending;
int multi_local_death_pending(void) { return local_death_pending; }
#endif
int Bounty_target = 0;
...
multi_make_ghost_player(int playernum)
{
	...
	multi_reset_player_object(obj);
#ifdef __ANDROID__
	/* A peer finishing the destination load can activate its ship after our
	 * portable merge. It is still the same living ship, not a respawn */
	if (coop_travel_blocks_gameplay()) {
		COOPLOG("travel player activated without resetting inventory: player=%d", playernum);
		return;
	}
#endif
...
#ifdef __ANDROID__
	const int defer_death = coop_travel_defer_player_death(killed_pnum);
#endif
...
multi_do_kill(const ubyte *buf)
{
...
#ifdef __ANDROID__
	if (pnum == Player_num) local_death_pending = 0;
#endif
	...
	if (Control_center_destroyed
#ifdef __ANDROID__
	    && !coop_travel_defer_player_death(pnum)
#endif
	) {
		reset_obs();
...
#ifdef __ANDROID__
	multi_log_hidden_door_state("recv_before", segnum, side, flag);
#endif
...
#ifdef __ANDROID__
		multi_log_hidden_door_state("recv_after_blast", segnum, side, flag);
#endif
		return;
	}
...
#ifdef __ANDROID__
	multi_log_hidden_door_state("recv_after", segnum, side, flag);
#endif
}
...
#ifdef __ANDROID__
	if ((Game_mode & GM_MULTI_COOP) && Netgame.AllowSecretWarps)
		COOPLOG("travel score received: player=%d score=%d level=%d frozen=%d", pnum,
		        Players[pnum].score, Current_level_num, coop_travel_blocks_gameplay());
#endif
...
// Send only notifications actually generated by the activating client
void multi_send_trigger_message(const char *message)
{
	ubyte buf[2 + MULTI_TRIGGER_MESSAGE_LEN] = {0};

	if (!(Game_mode & GM_MULTI_COOP) || is_observer())
		return;
	buf[0] = MULTI_COOP_TRIGGER_MESSAGE;
	buf[1] = (ubyte)Player_num;
	snprintf((char *)buf + 2, MULTI_TRIGGER_MESSAGE_LEN, "%s", message);
	multi_send_data(buf, sizeof(buf), 2);
}

static void multi_do_trigger_message(const ubyte *buf, int authenticated_sender)
{
	int pnum = buf[1];
	char message[MULTI_TRIGGER_MESSAGE_LEN];

	// Reliable notifications may be relayed by the host for another player

	if (!(Game_mode & GM_MULTI_COOP) || pnum >= N_players || pnum >= MAX_PLAYERS || pnum == Player_num ||
	    (authenticated_sender >= 0 && pnum != authenticated_sender &&
	     authenticated_sender != multi_who_is_master()) ||
	    Players[pnum].connected != CONNECT_PLAYING)
		return;
	memcpy(message, buf + 2, sizeof(message));
	message[sizeof(message) - 1] = '\0';
	HUD_init_message(HM_DEFAULT, "%s: %s", Players[pnum].callsign, message);
}
...
void multi_do_light (const ubyte *buf)
{
	...
	if (seg < 0 || seg > Highest_segment_index) {
		con_printf(CON_URGENT, "multi_do_light: invalid segment %d (highest %d)\n", seg, Highest_segment_index);
		return;
	}
	for (i=0;i<6;i++)
	{
...
{
#ifdef __ANDROID__
	/* Travel carries immutable player snapshots while the mine is frozen */
	if (coop_travel_blocks_gameplay()) return;
#endif

	fix afterburner_charge = GET_INTEL_INT(buf + 39);
	ushort secondary_weapon_flags = buf[29];
#ifdef __ANDROID__
	secondary_weapon_flags |= (ushort)buf[82] << 8;
#endif
	...
	if (is_observer())
	{
		...
		Players[buf[1]].secondary_weapon_flags = secondary_weapon_flags;
		...
		Players[buf[1]].afterburner_charge = afterburner_charge;
	...
	else if ((Game_mode & GM_MULTI_COOP) && buf[1] != Player_num)
	{
		...
		Players[pnum].secondary_weapon_flags = secondary_weapon_flags;
		Players[pnum].secondary_weapon = (sbyte)buf[30];
		Players[pnum].energy = GET_INTEL_INT(buf + 31);
		Players[pnum].afterburner_charge = afterburner_charge;
...
void multi_send_restore_game(ubyte slot, uint id)
{
	...
	if ((Game_mode & GM_MULTI_COOP) && multi_i_am_master()) {
		...
		if (transfer_status < 0) {
			COOPLOG("multi_send_restore_game: game=d2 restore transfer failed slot=%d id=%u file='%s'",
			        slot, id, filename);
			return;
		}
...
#ifdef __ANDROID__
	COOPLOG("multi_send_restore_game: game=d2 slot=%d id=%u current_level=%d net_level=%d n_players=%d",
...
void multi_restore_game(ubyte slot, uint id)
{
...
#ifdef __ANDROID__
	COOPLOG("multi_restore_game begin: game=d2 file='%s' slot=%d request_id=%u current_level=%d net_level=%d master=%d n_players=%d",
...
#ifdef __ANDROID__
	COOPLOG("multi_restore_game id check: game=d2 file_id=%d request_id=%u sentinel=%u",
	...
	COOPLOG("multi_restore_game done: game=d2 current_level=%d net_level=%d player_num=%d objnum=%d connected=%d",
...
#endif
{
	...
	switch(type)
	{
		...
		case MULTI_GUIDEBOT_GOAL:
			if (!Endlevel_sequence) escort_goal_message_receive(buf, authenticated_sender); break;
		case MULTI_COOP_TRIGGER_MESSAGE:
			if (!Endlevel_sequence) multi_do_trigger_message(buf, authenticated_sender); break;
		case MULTI_TRIGGER:
