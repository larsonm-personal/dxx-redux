...
void HUD_prepare_message_frame()
{
	...
	guided_message_visible =
		Guided_missile[Player_num] && Guided_missile[Player_num]->type == OBJ_WEAPON &&
...
void HUD_render_message_frame()
{
	...
	if (HUD_guided_message_y >= 0) {
		...
		gr_string(0x8000, HUD_guided_message_y, "Guided Missile View");
	}
#ifdef __ANDROID__
	if (HUD_restore_status_message) {
