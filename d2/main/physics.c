...
static void input_demo_log_replay_physics_fvi_fate(object *obj,
	int fate,
...
void do_physics_sim(object *obj)
{
	...
	*/
	...
	if (fate == HIT_WALL && hit_info->hit_side_seg >= 0 &&
		...
		"fate=%s start_seg=%d hit_seg=%d hit_side=%d hit_side_seg=%d child=%d wall=%d wall_type=%d wall_state=%d wall_flags=0x%x hit_obj=%d hit_type=%d hit_id=%d hit_sig=%d hit_obj_seg=%d hit_size=%d hit_obj_pos=(%d,%d,%d) pos=(%d,%d,%d) last=(%d,%d,%d) target=(%d,%d,%d) frame_vec=(%d,%d,%d) vel=(%d,%d,%d) size=%d flags=0x%x phys_flags=0x%x parent_type=%d parent=%d parent_sig=%d life=%d sim_time=%d ignore_count=%d",
			...
			if (!Robot_info[obj->id].thief && !Robot_info[obj->id].attack_type) {
				if (obj->ctype.ai_info.SKIP_AI_COUNT * FrameTime < 3*F1_0/4) {
					...
					input_demo_record_phys_apply_rot_event(obj, force_vec,
						skip_before, addval, skip_after, rate, vecmag, tval);
					if (input_demo_replay_homing_desync_probe_active()) {
						char probe[256];

						snprintf(probe, sizeof(probe),
							"skip_before=%d add=%d rate=%d vecmag=%d tval=%d force=(%d,%d,%d)",
							skip_before,
							addval,
							rate,
							vecmag,
							...
							force_vec->y,
							force_vec->z);
						input_demo_append_replay_probe_message("phys_apply_rot_rng",
							obj, probe);
					}
