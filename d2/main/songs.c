...
#ifdef ANDROID
				track_overlay_notify_mission_music(BIMSongs[songnum].filename, songnum);
#endif
			}
			break;
		}
...
int songs_play_level_song( int levelnum, int offset )
{
	...
	switch (GameCfg.MusicType)
	{
		...
		case MUSIC_TYPE_CUSTOM:
		{
			// FX RNG: music selection only, this just shuffles the custom soundtrack order
...
#ifdef __ANDROID__
/*
 * Unified track control for Android touch overlay -- works across all
 * music types (BUILTIN/MIDI, REDBOOK, CUSTOM/jukebox).
