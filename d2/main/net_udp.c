...
void net_udp_send_objects(void)
{
	...
	for (i = Network_send_objnum; i <= Highest_object_index; i++)
	{
		...
		object_rw_swap((object_rw *)&object_buffer[loc], 1, 1);
#endif
...
#ifdef WORDS_BIGENDIAN
				object_rw_swap((object_rw *)&data[loc], 1, 1);
#endif
...
void net_udp_read_endlevel_packet( ubyte *data, int data_len, struct _sockaddr sender_addr )
{
	...
	{
		if (memcmp((struct _sockaddr *)&Netgame.players[multi_who_is_master()].protocol.udp.addr, (struct _sockaddr *)&sender_addr, sizeof(struct _sockaddr)))
			return;
...
int net_udp_read_sync_packet( ubyte * data, int data_len, struct _sockaddr sender_addr )
{
...
#ifdef __ANDROID__
			if (Game_mode & GM_MULTI_COOP)
				crash_breadcrumb_v("d2 sync reject invalid_packet len=%d", data_len);
#endif
			return 0;
		}
...
#ifdef __ANDROID__
		if (Game_mode & GM_MULTI_COOP)
			crash_breadcrumb_v("d2 sync reject checksum remote=%u local=%u",
...
#ifdef __ANDROID__
				if (Game_mode & GM_MULTI_COOP)
					crash_breadcrumb_v("d2 sync reject duplicate_self slot=%d callsign=%.8s",
					                   i, temp_callsign);
#endif
				Network_status = NETSTAT_MENU;
				return 0;
	...
	if (Game_mode & GM_MULTI_COOP)
		crash_breadcrumb_v("d2 sync accepted player=%d players=%d callsign=%.8s",
...
#ifdef __ANDROID__
		if (Game_mode & GM_MULTI_COOP)
			crash_breadcrumb_v("d2 send_sync local_reject player=%d players=%d status=%d",
...
#ifdef __ANDROID__
#include "../../android/app/src/main/cpp/shared/net/net_udp_android_autonet_shared.h"
#endif /* __ANDROID__ */
...
net_udp_level_sync(void)
{
	...
	if (Game_mode & GM_MULTI_COOP) {
		COOPLOG("level_sync begin: game=d2 level=%d player=%d players=%d master=%d status=%d",
		        ...
		        Network_status);
		crash_breadcrumb_v("d2 coop net level_sync begin level=%d", Current_level_num);
	}
#endif
	...
	if (Game_mode & GM_MULTI_COOP) {
		...
		crash_breadcrumb_v("d2 coop net level_sync result=%d status=%d", result,
		                   Network_status);
	}
#endif
...
#ifdef __ANDROID__
		if (Game_mode & GM_MULTI_COOP)
			COOPLOG("level_sync closed session: game=d2 level=%d result=%d",
...
int net_udp_do_join_game(ubyte join_as_obs)
{
	...
	int can_join = net_udp_can_join_netgame(&Netgame, join_as_obs);
	switch (can_join)
	{
		case 0:
	...
	Difficulty_level = Netgame.difficulty;
	if(! join_as_obs) {
		/* android port: pick a temp slot that won't collide with the
		 * master -- see the matching comment in auto_join */
		int pnum = 1;
...
void net_udp_android_set_my_port(int my_port)
{
...
int net_udp_android_send_raw(const void *data, int len, const struct _sockaddr *addr)
{
	return dxx_sendto(UDP_Socket[0], data, len, 0,
		(const struct sockaddr *)addr, sizeof(struct _sockaddr));
}
...
#include "../../android/app/src/main/cpp/shared/net/net_udp_p2p_proxy_shared.h"
                             ...
                             int is_proxy)
{
	...
	if(their->player.observer) {
		if(Netgame.numobservers < Netgame.max_numobservers) {
			net_udp_welcome_player(
			    their, authenticated_player_num, 0, is_proxy);
		} else {
			net_udp_dump_player(their->player.protocol.udp.addr, their->token, DUMP_FULL);
		}
		return;
	...
	if (identity_match >= 0) {
		MPDIAG("refuse_stuff: '%s' matches existing player %d, welcoming\n",
		       their->player.callsign, identity_match);
		net_udp_welcome_player(
