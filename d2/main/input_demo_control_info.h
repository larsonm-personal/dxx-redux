#ifndef D2_INPUT_DEMO_CONTROL_INFO_H
#define D2_INPUT_DEMO_CONTROL_INFO_H
	...
	input_demo_control_pulse *pulse, const control_info *controls)
{
	...
	state->afterburner_state = controls->afterburner_state;
	state->energy_to_shield_state = controls->energy_to_shield_state;
	...
	pulse->toggle_bomb_count = controls->toggle_bomb_count;
	pulse->headlight_count = controls->headlight_count;
}
	...
	const input_demo_control_state *state, const input_demo_control_pulse *pulse)
{
	...
	controls->afterburner_state = state->afterburner_state;
	controls->energy_to_shield_state = state->energy_to_shield_state;
	...
	controls->toggle_bomb_count = pulse->toggle_bomb_count;
	controls->headlight_count = pulse->headlight_count;
}
