			...
			if (!(Players[Player_num].flags & PLAYER_FLAGS_CLOAKED))
				if (dist_to_player < robot->size + playerobj->size + F1_0*2) {
					...
					if (robptr->energy_drain && Players[Player_num].energy) {
						...
						snprintf(extra_json, sizeof(extra_json), ",\"robot_obj\":%d,\"robot_id\":%d,\"robot_signature\":%d,\"drain\":%d", (int)(robot - Objects), robot->id, robot->signature, robptr->energy_drain);
						snprintf(extra_log, sizeof(extra_log), " robot_obj=%d robot_id=%d robot_signature=%d drain=%d", (int)(robot - Objects), robot->id, robot->signature, robptr->energy_drain);
...
int do_robot_dying_frame(object *objp, fix64 start_time, fix roll_duration, sbyte *dying_sound_playing, int death_sound, fix expl_scale, fix sound_scale)
{
	...
	fix	roll_val, temp;
	fix	sound_duration;
	int translated_death_sound = death_sound >= 0 && death_sound < MAX_SOUNDS ? digi_xlat_sound(death_sound) : -1;
	...
	if (GameArg.SndDigiSampleRate && translated_death_sound >= 0 && translated_death_sound < Num_sound_files)
		sound_duration = fixdiv(GameSounds[translated_death_sound].length,GameArg.SndDigiSampleRate);
	else
		sound_duration = F1_0;
