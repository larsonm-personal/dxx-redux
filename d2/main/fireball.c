			...
			if ( (obj0p!=objp) && !(obj0p->flags&OF_SHOULD_BE_DEAD) && ((obj0p->type==OBJ_WEAPON && (obj0p->id==PROXIMITY_ID || obj0p->id==SUPERPROX_ID || obj0p->id==PMINE_ID)) || (obj0p->type == OBJ_CNTRLCEN) || (obj0p->type==OBJ_PLAYER) || ((obj0p->type==OBJ_ROBOT) && parent_check))) {
				...
				if ( dist < maxdistance ) {
							...
							case OBJ_ROBOT:
								...
								if (!d1_in_d2_use_d1_gameplay() && (objp != NULL) && (!Robot_info[obj0p->id].boss_flag) && (Weapon_info[objp->id].flash)) {
									...
									if (obj->ctype.ai_info.SKIP_AI_COUNT * FrameTime < F1_0) {
										...
										// SIM RNG: this directly feeds live robot stun spin physics
					...
					if ((input_demo_recorder_is_active() || input_demo_replay_is_loaded()) && obj0p->id == Player_num) {
						...
						if (objp != NULL && objp->type == OBJ_WEAPON)
							input_demo_log_player_weapon_hit(input_demo_debug_activity_mode_name(), input_demo_debug_frame_index(), objp, obj0p, damage, &pos_hit);
					}
...
//				new_pos.z += (d_rand()-16384)*6;
			...
			// SIM RNG: this decides whether the spawned robot also drops a shield pickup
			if (d_rand() > 16384)
		...
		if (objp->contains_type == OBJ_POWERUP)
		{
			if (objp->contains_id == POW_SHIELD_BOOST) {
				if (Players[Player_num].shields >= i2f(100)) {
					// SIM RNG: this decides whether the shield drop exists at all
					if (d_rand() > 16384) {
						return -1;
					}
				...
				if (Players[Player_num].energy >= i2f(100)) {
					// SIM RNG: this decides whether the energy drop exists at all
					if (d_rand() > 16384) {
						return -1;
					}
