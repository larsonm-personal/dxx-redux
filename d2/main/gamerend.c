...
static int input_demo_should_preserve_ui_rng(void)
{
	return input_demo_replay_is_loaded() ||
		(Newdemo_state == ND_STATE_RECORDING && input_demo_recorder_is_active());
}

static int input_demo_capture_preserved_ui_rng(unsigned int *saved_rng_state)
{
	return input_demo_should_preserve_ui_rng() && d_rand_get_state(saved_rng_state);
}

static void input_demo_restore_preserved_ui_rng(int preserve_rng, unsigned int saved_rng_state)
{
	if (preserve_rng)
