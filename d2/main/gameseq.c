...
{
	int i,k,j, real_start_count, generated_starts = 0, fallback_starts = 0;
	...
	if (Game_mode & GM_MULTI) {
		...
		for (; k < MAX_PLAYERS && real_start_count > 0; k++) {
			int source = k % real_start_count;

			if (Game_mode & GM_MULTI_COOP) {
				if (!coop_find_fanout_start(source, k, &Player_init[k].pos, &Player_init[k].segnum))
					fallback_starts++;
				Player_init[k].orient = Player_init[source].orient;
				generated_starts++;
			} else {
		...
		if ((Game_mode & GM_MULTI_COOP) && generated_starts)
			con_printf(CON_VERBOSE, "coop starts: found %d generated %d fallback %d\n", real_start_count, generated_starts, fallback_starts);
	}
}
...
void StartNewLevelSecret(int level_num, int page_in_textures)
{
	...
	if (First_secret_visit || (Newdemo_state == ND_STATE_PLAYBACK)) {
		/* Keep secret-level HUD counts separate from the saved parent mine */
		Players[Player_num].num_kills_level = 0;
		Players[Player_num].num_robots_level = count_number_of_robots();
		Players[Player_num].hostages_level = count_number_of_hostages();
		Players[Player_num].hostages_on_board = 0;
...
#ifdef __ANDROID__
	if (Game_mode & GM_MULTI_COOP) {
		COOPLOG("AdvanceLevel begin: game=d2 current=%d last=%d players=%d master=%d",
		        Current_level_num, Last_level, N_players, multi_i_am_master());
		crash_breadcrumb_v("d2 coop AdvanceLevel begin current=%d", Current_level_num);
	}
#endif
...
#ifdef __ANDROID__
		if (Game_mode & GM_MULTI_COOP)
			COOPLOG("AdvanceLevel endlevel sync: game=d2 current=%d result=%d players=%d",
...
#ifdef __ANDROID__
		if (Game_mode & GM_MULTI_COOP)
			COOPLOG("AdvanceLevel starting next: game=d2 current=%d next=%d players=%d",
			        Current_level_num, Next_level_num, N_players);
#endif
		StartNewLevel(Next_level_num);
#ifdef __ANDROID__
		if (Game_mode & GM_MULTI_COOP)
			COOPLOG("AdvanceLevel returned: game=d2 current=%d next=%d game_window=%d",
...
void StartNewLevelSub(int level_num, int page_in_textures, int secret_flag)
{
	...
	if (Game_mode & GM_MULTI_COOP) {
		COOPLOG("StartNewLevelSub begin: game=d2 requested=%d current=%d page=%d players=%d master=%d",
		        ...
		        multi_i_am_master());
		crash_breadcrumb_v("d2 coop StartNewLevelSub begin level=%d", level_num);
	}
#endif
	if (!(Game_mode & GM_MULTI)) {
		last_drawn_cockpit = -1;
   ...
   BigWindowSwitch=0;
...
#ifdef __ANDROID__
	if (Game_mode & GM_MULTI_COOP) {
		COOPLOG("StartNewLevelSub loaded: game=d2 requested=%d current=%d objects=%d segments=%d starts=%d",
		        level_num, Current_level_num, Highest_object_index + 1,
		        Highest_segment_index + 1, NumNetPlayerPositions);
		crash_breadcrumb_v("d2 coop level loaded level=%d", Current_level_num);
	}
#endif
	...
	if (Game_mode & GM_NETWORK)
	{
...
#ifdef __ANDROID__
		if (Game_mode & GM_MULTI_COOP) {
			COOPLOG("StartNewLevelSub sync begin: game=d2 level=%d player=%d players=%d master=%d",
		...
		if (Game_mode & GM_MULTI_COOP) {
			...
			crash_breadcrumb_v("d2 coop level sync done level=%d result=%d player=%d players=%d status=%d",
...
#ifdef __ANDROID__
	if (Game_mode & GM_MULTI_COOP)
		crash_breadcrumb_v("d2 coop init player_stats done player=%d", Player_num);
#endif
...
#ifdef __ANDROID__
	if (Game_mode & GM_MULTI_COOP)
		crash_breadcrumb_v("d2 coop init multi_prep done player=%d objects=%d",
...
#ifdef __ANDROID__
	if (Game_mode & GM_MULTI_COOP)
		crash_breadcrumb_v("d2 coop init subsystems done player=%d obj=%d",
		                   Player_num,
...
#ifdef __ANDROID__
		if (Game_mode & GM_MULTI_COOP)
			crash_breadcrumb_v("d2 coop StartLevel enter player=%d", Player_num);
#endif
...
#ifdef __ANDROID__
	if (Game_mode & GM_MULTI_COOP)
		crash_breadcrumb_v("d2 coop StartLevel done player=%d", Player_num);
#endif
	...
	if (Game_mode & GM_MULTI_COOP) {
		COOPLOG("StartNewLevelSub ready: game=d2 level=%d player=%d objects=%d game_window=%d",
		        ...
		        Game_wind != NULL);
		crash_breadcrumb_v("d2 coop StartNewLevelSub ready level=%d", Current_level_num);
	}
#endif
}
