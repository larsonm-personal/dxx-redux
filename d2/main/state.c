...
// 29- Save generated secret-area found bits
...
static int state_android_physfs_raw_eof(struct PHYSFS_File *file)
{
	return PHYSFS_eof(file);
}

static PHYSFS_sint64 state_android_physfs_raw_read(struct PHYSFS_File *file,
	void *buffer,
	PHYSFS_uint32 obj_size,
	PHYSFS_uint32 obj_count)
{
	return PHYSFS_readBytes(file, buffer, (PHYSFS_uint64)obj_size * obj_count) / obj_size;
}

static PHYSFS_sint64 state_android_physfs_raw_write(struct PHYSFS_File *file,
	const void *buffer,
	PHYSFS_uint32 obj_size,
	PHYSFS_uint32 obj_count)
{
	return PHYSFS_writeBytes(file, buffer, (PHYSFS_uint64)obj_size * obj_count) / obj_size;
}

static int state_android_physfs_raw_close(struct PHYSFS_File *file)
{
...
static PHYSFS_sint64 state_android_physfs_raw_read(struct PHYSFS_File *file,
	...
	PHYSFS_uint32 obj_count)
{
	return PHYSFS_read(file, buffer, obj_size, obj_count);
}
...
static PHYSFS_sint64 state_android_physfs_raw_write(struct PHYSFS_File *file,
	...
	PHYSFS_uint32 obj_count)
{
	return PHYSFS_write(file, buffer, obj_size, obj_count);
}
...
int copy_file(char *old_file, char *new_file)
{
	sbyte	*buf;
	int		buf_size;
	struct PHYSFS_File *in_file, *out_file;
	...
	buf_size = state_android_physfs_raw_length(in_file);
	while (buf_size && !(buf = d_malloc(buf_size)))
		buf_size /= 2;
	if (buf_size == 0)
		return -5;	// likely to be an empty file

	while (!state_android_physfs_raw_eof(in_file))
	{
		int bytes_read;

		bytes_read = state_android_physfs_raw_read(in_file, buf, 1, buf_size);
		if (bytes_read < 0)
			Error("Cannot read from file <%s>: %s", old_file, PHYSFS_getLastError());

		Assert(bytes_read == buf_size || state_android_physfs_raw_eof(in_file));

		if (state_android_physfs_raw_write(out_file, buf, 1, bytes_read) < bytes_read)
			Error("Cannot write to file <%s>: %s", new_file, PHYSFS_getLastError());
	}

	d_free(buf);

	if (!state_android_physfs_raw_close(in_file))
	{
		state_android_physfs_raw_close(out_file);
		return -3;
	}

	if (!state_android_physfs_raw_close(out_file))
		return -4;
...
// Read Coop Info
	...
	android_profile_log_restore("d2", Current_level_num, filename, Game_mode,
