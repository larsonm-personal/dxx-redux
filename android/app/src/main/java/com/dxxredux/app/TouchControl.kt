...
fun applyResponseCurve(input: Float, curve: ResponseCurve, exponent: Float = 2f): Float {
...
data class FloatingZone(
    ...
    fun toJson() = JSONObject().apply {
        put("left", leftPct.toDouble()); put("top", topPct.toDouble())
        put("right", rightPct.toDouble()); put("bottom", bottomPct.toDouble())
    }
    companion object {
        fun fromJson(j: JSONObject) = FloatingZone(
...
data class AnalogStickControl(
    ...
    fun toJson() = JSONObject().apply {
        put("type", "analog_stick"); put("id", id)
        put("x", xPct.toDouble()); put("y", yPct.toDouble())
        put("size", sizeMult.toDouble()); put("opacity", opacity.toDouble())
        put("axisX", axisX); put("axisY", axisY)
        put("invertX", invertX); put("invertY", invertY)
    ...
    companion object {
        fun fromJson(j: JSONObject) = AnalogStickControl(
            id = j.getString("id"),
            xPct = j.getDouble("x").toFloat(), yPct = j.getDouble("y").toFloat(),
            sizeMult = j.optDouble("size", 1.0).toFloat(),
            opacity = j.optDouble("opacity", 0.7).toFloat(),
            axisX = j.getInt("axisX"), axisY = j.getInt("axisY"),
            invertX = j.optBoolean("invertX"), invertY = j.optBoolean("invertY"),
            ...
            floatingZone = if (j.has("floatingZone")) FloatingZone.fromJson(j.getJSONObject("floatingZone")) else FloatingZone(),
...
data class ButtonControl(
    ...
    fun toJson() = JSONObject().apply {
        put("type", "button"); put("id", id)
        put("x", xPct.toDouble()); put("y", yPct.toDouble())
        put("size", sizeMult.toDouble()); put("opacity", opacity.toDouble())
        put("binding", binding)
        put("label", label); put("shape", shape.name)
        put("toggle", toggle); put("haptic", hapticFeedback)
    }
    companion object {
        fun fromJson(j: JSONObject) = ButtonControl(
            id = j.getString("id"),
            xPct = j.getDouble("x").toFloat(), yPct = j.getDouble("y").toFloat(),
            sizeMult = j.optDouble("size", 1.0).toFloat(),
            opacity = j.optDouble("opacity", 0.7).toFloat(),
...
data class SliderControl(
    ...
    fun toJson() = JSONObject().apply {
        put("type", "slider"); put("id", id)
        put("x", xPct.toDouble()); put("y", yPct.toDouble())
        put("size", sizeMult.toDouble()); put("opacity", opacity.toDouble())
        put("axis", axis); put("orientation", orientation.name)
        put("responseCurve", responseCurve.name)
        put("exponent", exponent.toDouble())
        put("sensitivity", sensitivity.toDouble())
    ...
    companion object {
        fun fromJson(j: JSONObject) = SliderControl(
            id = j.getString("id"),
            xPct = j.getDouble("x").toFloat(), yPct = j.getDouble("y").toFloat(),
            sizeMult = j.optDouble("size", 1.0).toFloat(),
            opacity = j.optDouble("opacity", 0.7).toFloat(),
...
data class RadialMenuControl(
    ...
    fun toJson() = JSONObject().apply {
        put("type", "radial_menu"); put("id", id)
        put("x", xPct.toDouble()); put("y", yPct.toDouble())
        put("size", sizeMult.toDouble()); put("opacity", opacity.toDouble())
    ...
    companion object {
        fun fromJson(j: JSONObject) = RadialMenuControl(
            id = j.getString("id"),
            xPct = j.getDouble("x").toFloat(), yPct = j.getDouble("y").toFloat(),
            sizeMult = j.optDouble("size", 1.0).toFloat(),
            opacity = j.optDouble("opacity", 0.7).toFloat(),
...
data class DPadControl(
    ...
    val upBinding: Int, val downBinding: Int,
    val leftBinding: Int, val rightBinding: Int,
    val swipeThreshold: Float = TouchBindings.DEFAULT_SWIPE_THRESHOLD,
    val hapticFeedback: Boolean = true
) {
    fun toJson() = JSONObject().apply {
        put("type", "dpad"); put("id", id)
        put("x", xPct.toDouble()); put("y", yPct.toDouble())
        put("size", sizeMult.toDouble()); put("opacity", opacity.toDouble())
        put("mode", mode.name)
        put("up", upBinding); put("down", downBinding)
        put("left", leftBinding); put("right", rightBinding)
        put("swipeThreshold", swipeThreshold.toDouble())
        put("haptic", hapticFeedback)
    }
    companion object {
        fun fromJson(j: JSONObject) = DPadControl(
            id = j.getString("id"),
            xPct = j.getDouble("x").toFloat(), yPct = j.getDouble("y").toFloat(),
            sizeMult = j.optDouble("size", 1.0).toFloat(),
            opacity = j.optDouble("opacity", 0.7).toFloat(),
            mode = DPadMode.valueOf(j.optString("mode", "INDIVIDUAL")),
            upBinding = j.getInt("up"), downBinding = j.getInt("down"),
            leftBinding = j.getInt("left"), rightBinding = j.getInt("right"),
            swipeThreshold = j.optDouble("swipeThreshold", TouchBindings.DEFAULT_SWIPE_THRESHOLD.toDouble()).toFloat(),
            hapticFeedback = j.optBoolean("haptic", true)
        )
    }
}
...
data class GyroConfig(
    ...
    fun toJson() = JSONObject().apply {
        put("enabled", enabled); put("activation", activation.name)
        put("sensitivityX", sensitivityX.toDouble()); put("sensitivityY", sensitivityY.toDouble())
        put("invertX", invertX); put("invertY", invertY)
        put("axisX", axisX); put("axisY", axisY)
...
data class TouchLayout(
    ...
    fun toJson() = JSONObject().apply {
        put("version", version); put("name", name)
    ...
    companion object {
        fun fromJson(j: JSONObject): TouchLayout {
            fun <T> parseArray(key: String, parse: (JSONObject) -> T): List<T> {
