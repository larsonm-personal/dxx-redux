...
static int extend_4to8(int v)
{
	return (v << 4) | v;
}
static int extend_5to8(int v)
{
	return (v << 3) | (v >> 2);
}
static int extend_6to8(int v)
{
	return (v << 2) | (v >> 4);
}
static int extend_7to8(int v)
{
	return (v << 1) | (v >> 6);
}

/* Sign-extend a 3-bit value */
static int sign_extend_3(int v)
{
	return (v & 4) ? (v | ~7) : v;
}
...
static void decode_etc2_rgb_block(const uint8_t *src, uint8_t *out, int stride)
{
	...
	if (!diff) {
		...
		if (R2 < 0 || R2 > 31) {
			 ...
			 * 3: color2 - dist */
			int colors[4][3];
			colors[0][0] = clamp255(hr1 + dist);
			colors[0][1] = clamp255(hg1 + dist);
			colors[0][2] = clamp255(hb1 + dist);
			colors[1][0] = clamp255(hr1 - dist);
			colors[1][1] = clamp255(hg1 - dist);
			colors[1][2] = clamp255(hb1 - dist);
			colors[2][0] = clamp255(hr2 + dist);
			colors[2][1] = clamp255(hg2 + dist);
			colors[2][2] = clamp255(hb2 + dist);
			colors[3][0] = clamp255(hr2 - dist);
			colors[3][1] = clamp255(hg2 - dist);
			colors[3][2] = clamp255(hb2 - dist);
