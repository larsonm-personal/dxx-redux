...
static hud_counts_debug_state Hud_counts_debug;

void hud_counts_debug_reset(void)
{
	memset(&Hud_counts_debug, 0, sizeof(Hud_counts_debug));
}

const hud_counts_debug_state *hud_counts_get_debug_state(void)
{
	return &Hud_counts_debug;
}

static void hud_counts_debug_set_rect(hud_counts_debug_row *row,
                                       ...
                                       hud_counts_right_inset_fn right_inset, hud_counts_debug_row *debug_row)
{
	...
	gr_get_string_drawn_size(text, &w, &h);
	x = grd_curcanv->cv_bitmap.bm_w - right_inset(y, h) - w - FSPACX(1);
	hud_counts_debug_set_rect(debug_row, x, y, w, h);
                                     ...
                                     hud_counts_right_inset_fn right_inset, hud_counts_debug_row *debug_row)
{
	...
	if (lost <= 0) {
		...
		hud_counts_draw_right_text(y, full, right_inset, debug_row);
		return;
	}
	...
	gr_get_string_drawn_size(full, &full_w, &full_h);
	gr_get_string_drawn_size(label, &label_w, &label_h);
	gr_get_string_drawn_size(lost_str, &lost_w, &lost_h);

	x = grd_curcanv->cv_bitmap.bm_w - right_inset(y, full_h) - full_w - FSPACX(1);
	hud_counts_debug_set_rect(debug_row, x, y, full_w, full_h);
	if (HUD_message_area_intersects(x, y, full_w, full_h))
		return;
	debug_row->drawn = 1;
	if (Color_0_31_0 == -1)
		Color_0_31_0 = BM_XRGB(0, 31, 0);
                                    ...
                                    hud_counts_right_inset_fn right_inset, hud_counts_debug_row *debug_row)
{
	...
	hud_counts_draw_right_text(y, secret_str, right_inset, debug_row);
}
