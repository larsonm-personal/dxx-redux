...
static int hash_file_bytes(const char *path, char sha_hex[41])
{
	...
	for (;;) {
		...
		if (n == 0)
			break;
		dxx_sha1_update(&sha_ctx, buf, (unsigned int) n);
	}
	close_fd(fd);

	dxx_sha1_final(&sha_ctx, sha_digest);
	dxx_sha1_hex(sha_digest, sha_hex);
	return 0;
}
	...
	if (is_iso) {
		...
		for (i = 0; i < disc.num_tracks; i++) {
			...
			for (s = 0; s < t->num_sectors; s++) {
				...
				dxx_sha1_update(&sha_ctx, sector_buf, (size_t) t->sector_size);
			}
