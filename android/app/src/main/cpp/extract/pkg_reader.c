...
static int pkg_scan_cpio(pkg_archive_t *arc) {
    ...
    while ((ret = cpio_read_entry(&gs, &entry)) == 1) {
        ...
        /* Skip file data */
        if (entry.filesize > 0) {
            if (gz_skip(&gs, entry.filesize) < 0) { ret = -1; break; }
        }
    }
...
int pkg_extract_all(pkg_archive_t *arc, const char *output_dir,
    ...
    while ((ret = cpio_read_entry(&gs, &entry)) == 1) {
        ...
        while (remaining > 0) {
            ...
            if (got <= 0) { ok = 0; break; }
            if (fwrite(buf, 1, (size_t)got, fp) != (size_t)got) { ok = 0; break; }
