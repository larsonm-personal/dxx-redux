...
class LevelMetadataResultCacheTest {
    ...
    fun `published result is reused and source changes invalidate its identity`() {
        ...
        val routeFile = File(checkNotNull(root.parentFile), "d2x-redux/route-cache/g3/test.bin").apply {
    ...
    private fun resultJson(game: String): String =
        """{"status":"ok","source":"Level","game":"$game","levels":[{"status":"ok","route_readiness":"complete","route_cache_file":"route-cache/g3/test.bin"}]}"""
}
