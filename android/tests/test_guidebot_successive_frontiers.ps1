...
& (Join-Path $repoRoot 'android\helpers\regenerate_all_guidebot_simulations.ps1') `
    -MissionJson 'Descent.json' -Level 7 -Repeat 2 -OutputRoot $outputRoot -NoBuild:$NoBuild
if ($LASTEXITCODE -ne 0) { throw 'Successive-frontier simulations reported infrastructure failures' }
$record = Get-Content -LiteralPath (Join-Path $outputRoot 'results\Descent.simulation.json') -Raw | ConvertFrom-Json
