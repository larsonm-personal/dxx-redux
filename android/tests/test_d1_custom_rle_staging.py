...
SOURCE = (REPO_ROOT / "d2/main/d1_in_d2/d1_custom.c").read_text(encoding="utf-8")
...
class D1CustomRleStagingTest(unittest.TestCase):
    def test_rle_validation_precedes_staged_ownership(self) -> None:
        ...
        self.assertIn("info->rle_big", body)
        validate = body.index("bounded_rle_validate_bitmap")
        publish = body.index("info->data = data")
        self.assertLess(validate, publish)

    def test_custom_reader_has_no_live_bitmap_or_sound_commit(self) -> None:
        # Real malformed-file and buffer-lifetime coverage is in the C++ fixture
        self.assertNotIn("GameBitmaps", SOURCE)
        self.assertNotIn("GameSounds", SOURCE)
        self.assertNotIn("piggy_bitmap_set_file_state", SOURCE)
