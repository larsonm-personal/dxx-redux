...
static int expect_weapon_selector_domain(void)
{
	input_demo_control_record record;
	std::string error;
	const int invalid_values[] = { 11, 254, 255 };
	int value;
	size_t i;

	for (value = 0; value <= 10; ++value) {
		const std::string line = "{\"f\":1,\"p\":{\"sw\":" + std::to_string(value) + "}}";
		if (!input_demo_control_record_parse_json_line(
		        line, input_demo_control_info_game(), &record, &error))
			return report_failure_string(std::string("valid weapon selector failed: ") + error);
		if (!record.pulse.has_select_weapon_count || record.pulse.select_weapon_count != value)
			return report_failure("valid weapon selector changed during parsing");
	}
	for (i = 0; i != sizeof(invalid_values) / sizeof(invalid_values[0]); ++i) {
		...
		if (input_demo_control_record_parse_json_line(
		        line, input_demo_control_info_game(), &record, &error))
			return report_failure("out-of-domain weapon selector unexpectedly validated");
	}
	return 0;
}

static int expect_frame_time_domain(void)
{
	input_demo_control_record record;
	std::string error;
	const int valid_values[] = { 1, 3276, INPUT_DEMO_FRAME_TIME_MAX };
	const char *invalid_values[] = {
		"-2147483648", "-1", "0", "65537", "2147483647",
		"2147483648", "4294967295", "9223372036854775808", "18446744073709551615"
	};
	size_t i;

	for (i = 0; i != sizeof(valid_values) / sizeof(valid_values[0]); ++i) {
		const std::string line = "{\"f\":1,\"ft\":" + std::to_string(valid_values[i]) + "}";
		if (!input_demo_control_record_parse_json_line(
		        line, input_demo_control_info_game(), &record, &error))
			return report_failure_string(std::string("valid frame time failed: ") + error);
		if (!record.has_frame_time || record.frame_time != valid_values[i])
			return report_failure("valid frame time changed during parsing");
	}
	for (i = 0; i != sizeof(invalid_values) / sizeof(invalid_values[0]); ++i) {
		const std::string line = std::string("{\"f\":1,\"ft\":") + invalid_values[i] + "}";
		if (input_demo_control_record_parse_json_line(
		        line, input_demo_control_info_game(), &record, &error))
			return report_failure("out-of-domain frame time unexpectedly validated");
	}
	return 0;
}
...
int main(int argc, char *argv[])
{
	...
	if (expect_zero_release_update())
		return 1;
	if (expect_weapon_selector_domain())
		return 1;
	if (expect_frame_time_domain())
		return 1;
