...
static int test_retry_recovery_allows_only_authoritative_coop_companion(void)
{
	...
	CHECK(escort_retry_recovery_allowed(0, 0, 1, 0));
	CHECK(escort_retry_recovery_allowed(1, 1, 1, 1));
	CHECK(!escort_retry_recovery_allowed(1, 1, 1, 0));
	CHECK(!escort_retry_recovery_allowed(1, 1, 0, 1));
	CHECK(!escort_retry_recovery_allowed(1, 0, 1, 1));
	return 1;
}
