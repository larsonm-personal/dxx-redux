...
$output = Join-Path $repoRoot ('android/temp/test_diehard_countdown_switch/' + (Get-Date -Format 'yyyyMMdd_HHmmss'))
& (Join-Path $repoRoot 'android/helpers/regenerate_all_guidebot_simulations.ps1') `
    -RoutingDevelopmentSet -Level 17 -LevelFileFilter DH17.RL2 -Repeat 2 `
...
$level = @($aggregate.levels | Where-Object level_num -EQ 17)
...
if ($files.Count -ne 2) { throw 'Expected two Die Hard level 17 simulations' }
foreach ($file in $files) {
    ...
    if ($log -notmatch '(?s)complete step=.*label=Boss robot.*complete step=.*label=Shoot switch trigger 28.*complete step=.*label=Exit' -or
        $log -notmatch 'verified switch shot actor_seg=660 target_seg=664 wall=152' -or
        $log -match 'guided missile') {
        throw 'The switch must be shot after the boss opens its shutter'
    }
}
if ((Get-FileHash $files[0].FullName).Hash -ne (Get-FileHash $files[1].FullName).Hash) {
    throw 'Die Hard physical repeats differ'
}
Write-Host 'PASS repeated full-radius Die Hard level 17 completion through a switch behind a countdown door'
