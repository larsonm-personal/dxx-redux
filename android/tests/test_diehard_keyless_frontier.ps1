...
$output = Join-Path $repoRoot ('android/temp/test_diehard_keyless_frontier/' + (Get-Date -Format 'yyyyMMdd_HHmmss'))
& (Join-Path $repoRoot 'android/helpers/regenerate_all_guidebot_simulations.ps1') `
    -RoutingDevelopmentSet -Level 12 -LevelFileFilter DH12.RL2 -Repeat 2 `
...
$level = @($aggregate.levels | Where-Object level_num -EQ 12)
...
if ($files.Count -ne 2) { throw 'Expected two Die Hard level 12 simulations' }
foreach ($file in $files) {
    ...
    if ($log -notmatch '(?s)complete step=.*label=Shoot switch trigger 14.*complete step=.*label=Shoot switch trigger 13.*complete step=.*label=red key.*complete step=.*label=Boss robot.*complete step=.*label=Exit' -or
        $log -notmatch 'verified switch shot actor_seg=332 target_seg=342 wall=77' -or
        $log -match 'guided missile') {
        throw 'The route must reach the planned switch before acquiring its first key'
    }
}
if ((Get-FileHash $files[0].FullName).Hash -ne (Get-FileHash $files[1].FullName).Hash) {
    throw 'Die Hard physical repeats differ'
}
Write-Host 'PASS repeated full-radius Die Hard level 12 completion through a triggered keyless door'
