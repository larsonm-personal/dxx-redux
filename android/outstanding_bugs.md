...
# bugs
[ ] "exit" overlay button has no effect in some cases (for example, a waiting player screen for multiplayer), and I think also during cutscenes. it may have no effect at all anymore, I can't tell. can you take a look?
[ ] "start game" green overlay button is now selecting/deselecting players from the "select up to N players" screen rather than actually starting the game. some signal may have gotten mixed up during a recent menu refactoring
[ ] auto save thumbnails are still not shown sometimes. it could be because they're not created well enough during program exit, or could be that the save thumnail reader is flawed. either way, I'd like to make thumnail failures more rare
[ ] cutscene videos sometimes play with corrupted borders (when the video doesn't fill the entire screen). can you look into that? it might need the borders drawn on each frame, or zeroed before the video starts
