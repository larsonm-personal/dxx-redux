...
## Castaway 9 boss encounter follow-up

Verify authored boss-room geometry, visibility and activation before changing routing or simulation AI. Trace normal engine teleport eligibility and a legitimate encounter point; preserve honest incomplete status unless a physical completion can be demonstrated. Add focused coverage and rerun core regressions for any retained fix. Counterstrike -5 remains last.

Encounter trace finds two bosses: object 2 in sealed segment 43 and object 4 in reachable segment 106. The engine's do_boss_dying_frame opens the exit after either boss death. Planner discovery retained only the first boss; now retain all and try another encounter if the earlier one is unreachable. A second fault in live certification replaced the selected second boss with the first; carry the selected object into the existing live object slot and preserve that identity. Physical diagnostic now defeats the second boss through the normal death transition and exits at frame 4210. No boss teleport or AI sandbox change required; temporary diagnostics removed.

Final Castaway 9 validation: both Windows engines build (temp/cast9_final_build.log); all 49 native tests pass (temp/cast9_verified_ctest.log); scoped quality and diff checks pass. New test_castaway_level9_second_boss_route.ps1 verifies two identical physical completions at frame 4210, selected boss segment 106, all objectives and exit, with metadata agreement. Standalone JSON retains key_carrier_objnum only for key objectives; internal primary-object identity stays in the existing live slot. Final metadata refresh changes only Castaway 9, partial to ok (android/temp/cast9_final_metadata). Full run android/temp/core_four_cast9_verified has 86/88 ok, no regressions, and only Castaway 9 simulation changed. Refreshed Castaway metadata and simulation JSON. All ten Castaway levels now pass. Remaining: Obsidian 10 next, Counterstrike secret -5 last. Android device behavior was not tested.

## Obsidian 10 follow-up

Obsidian 10 switch 0 is in recess 63; its useful outside firing position is behind yellow door 20. Alternate switch 14 at 503 opens access to that outside region and is itself behind yellow door 138. The shared planner now searches bounded center firing positions with independent prerequisites when the source room is inaccessible, excluding the target switch from its own approach. Conditional shots can acquire and explicitly open a keyed door. Preserve the existing firing-search order and source approach for accessible rooms; broader speculative searches and retries caused metadata budget regressions and were removed.

Live certification retains a canonical firing position behind an owned-key door instead of substituting an unrelated approximate waypoint, provided the player has clearance through that route. This preserves Castaway 6's narrow-clearance guidance. The D2 path follower also checks the following leg before consuming a neighboring segment waypoint, preventing the Obsidian 10 corner stall while retaining ordinary same-destination-segment advancement.

Full corpus validation exposed a separate Obsidian 14 simulation stall: after a fly-through trigger, the actor left the newly installed path and stayed aimed at an unreachable waypoint. After two seconds without progress, the simulation now checks whether the actor is outside its path and rebuilds the same physical target from its actual segment. It uses the normal route path builder, preserves the semantic objective, and does not reset either timeout. Broader shot-alignment restrictions and speculative retry experiments were removed.

Both Windows engines build (temp/obs10_verified_build.log), all 49 native tests pass (temp/obs10_verified_ctest.log), and scoped quality checks pass. test_obsidian_level10_firing_dependencies.ps1 confirms two identical runs of Obsidian 10 (6641 frames) and Obsidian 14 (9058 frames), with all objectives, exits and metadata agreement. Host metadata refresh android/temp/obs10_waypoint_metadata has no previously passing route-status regressions; only Castaway 2 and Obsidian 10/11/13 metadata change. Final full-corpus results are recorded below. Android device behavior was not tested.

Final corpus: android/temp/core_four_obs10_final passes 87/88 with zero previously passing regressions: First Strike 30/30, Counterstrike 29/30, Castaway 10/10, Obsidian 18/18. Refreshed all four simulation JSON files after comparing every level against the 86/88 Castaway 9 baseline. Remaining failure: Counterstrike secret -5, kept last as requested.
