...
- add `-inputdemo-replay <demo.json5>` to D2 startup
...
- [x] Add D2 `-inputdemo-replay <demo.json5>` startup handling in `inferno.c`
...
- D2 accepts `-inputdemo-replay <demo.json5>` and starts the requested mission and level
...
- added D2 runtime replay startup in `inferno.c` by extending the existing metadata-validation path with `-inputdemo-replay <demo.json5>`
