...
| GQ1-CHUNK-0168 | [x] DONE | source | high | authored-source | `3 related paths` | 570 review lines under android/app/src/main/cpp | `GQD-0061`, `GQC-0181`; branch-added shared route boundary retained with no new quality root |
| GQ1-CHUNK-0169 | [x] DONE | source | high | authored-source | `4 related paths` | 453 review lines under android/app/src/main/cpp | `GQD-0062`, `GQC-0182`; deferred post-init seam confirms `GQD-0010`, quality duplicates `BR-0029` |
| GQ1-CHUNK-0170 | [x] DONE | source | high | authored-source | `4 related paths` | 609 review lines under android/app/src/main/cpp | `GQD-0063`, `GQC-0183`; retained shared OGL boundary; extended `GQF-0007`, `GQF-0020`, `GQF-0021` |
| GQ1-CHUNK-0171 | [x] DONE | source | high | authored-source | `4 related paths` | 648 review lines under android/app/src/main/cpp | `GQD-0064`, `GQC-0184`; retained shared rewind/cache boundary; extended `BR-0208`, `BR-0332`, `BR-0259` |
| GQ1-CHUNK-0172 | [x] DONE | source | high | authored-source | `4 related paths` | 750 review lines under android/app/src/main/cpp | `GQD-0065`, `GQC-0185`; retained shared RNG/result boundary; extended `BR-0246`, `BR-0247`, `BR-0222` |
| GQ1-CHUNK-0173 | [x] DONE | source | high | authored-source | `4 related paths` | 694 review lines under android/app/src/main/cpp | `GQD-0066`, `GQC-0186`; retained shared playsave boundary; extended `BR-0236`, `BR-0029` |
| GQ1-CHUNK-0174 | [x] DONE | source | high | authored-source | `4 related paths` | 749 review lines under android/app/src/main/cpp | `GQD-0067`, `GQC-0187`; retained shared debug/MIDI boundary; extended `BR-0238`, `BR-0239`, `BR-0394` |
| GQ1-CHUNK-0175 | [x] DONE | source | high | authored-source | `4 related paths` | 234 review lines under android/app/src/main/cpp | `GQD-0068`, `GQC-0188`; no inherited effect; extended `BR-0016`, `BR-0276` with seek fixes closed |
| GQ1-CHUNK-0176 | [x] DONE | source | high | authored-source | `4 related paths` | 529 review lines under android/app/src/main/cpp | `GQD-0069`, `GQC-0189`; retained shared auto-network boundary; extended `BR-0257`, `GQF-0023` |
| GQ1-CHUNK-0177 | [x] DONE | source | high | authored-source | `4 related paths` | 417 review lines under android/app/src/main/cpp | `GQD-0070`, `GQC-0190`; retained shared MSAA/shader boundary; extended `BR-0251`, `BR-0647` |
| GQ1-CHUNK-0178 | [x] DONE | source | high | authored-source | `4 related paths` | 647 review lines under android/app/src/main/cpp | `GQD-0071`, `GQC-0191`; no inherited effect; extended `GQF-0058`, `BR-0286`; archived `BR-0252` remains closed |
| GQ1-CHUNK-0179 | [x] DONE | source | high | authored-source | `4 related paths` | 424 review lines under android/app/src/main/cpp | `GQD-0072`, `GQC-0192`; retained shared play-save transaction boundary; extended `BR-0029`, `BR-0236` |
| GQ1-CHUNK-0180 | [x] DONE | source | high | authored-source | `4 related paths` | 278 review lines under android/app/src/main/cpp | `GQD-0073`, `GQC-0193`; retained shared validation/diagnostic boundary; extended `BR-0243`; archived `BR-0389` remains closed |
| GQ1-CHUNK-0181 | [x] DONE | source | high | authored-source | `4 related paths` | 676 review lines under android/app/src/main/cpp | `GQD-0074`, `GQC-0194`; retained shared font/graphics boundary; extended `BR-0029`, `BR-0236`, `GQF-0141`, `GQF-0142` |
...
| `GQD-0061` | `GQ1-CHUNK-0168` | `NO_INHERITED_EFFECT` | The route-planner model, C facade tail, snapshot summary API, shared consumers, tests, and paired secret-area facade headers are branch-added. Inherited engine sites retain compact game-local observations and calls; moving shared route policy into them would duplicate ownership and removes 0 base-existing paths, hunks, or lines |
| `GQD-0062` | `GQ1-CHUNK-0169` | `DEFER` | Duplicate confirmation of `GQD-0010`: paired inherited `songs.c` post-initialization name-table and diagnostic blocks could move behind the existing shared songs owner and remove about 16 additions across two paths, but no complete hunk. Startup-resume sites are compact ordered calls. Recheck only with adjacent songs work that reaches 30 inherited lines or removes a complete hunk |
| `GQD-0063` | `GQ1-CHUNK-0170` | `RETAIN` | Branch-added OGL texture and viewport owners already absorbed the duplicated policy, including a completed 92-line DMR1 extension-lookup extraction. Remaining paired inherited `ogl.c` sites are compact GL state-bearing calls; callback replacement would retain those observation points and add a larger interface without material path, hunk, or line reduction |
| `GQD-0064` | `GQ1-CHUNK-0171` | `RETAIN` | Branch-added rewind and route-cache owners already centralize shared policy. Paired inherited serializer declarations and cache build/facade sites are compact game-local ABI, state, and registration seams; moving them behind callbacks retains those sites and enlarges the shared interface. RGBA has no inherited consumer, so no material inherited path or complete hunk is removable |
| `GQD-0065` | `GQ1-CHUNK-0172` | `RETAIN` | Branch-added result, RNG-mode, and RNG-trace owners already centralize shared behavior. Paired inherited sites are local RNG, lifecycle, serializer, and simulation observations; callback replacement retains them and adds dispatch state. No material inherited path or complete hunk reduction is supported |
| `GQD-0066` | `GQ1-CHUNK-0173` | `RETAIN` | Branch-added playsave owners already centralize Android defaults, layout, and patch policy. Paired inherited `playsave.c` sites retain game-specific format versions and serialization order with compact includes, registrations, and calls. Moving those branches expands inherited change and violates format ownership, so no material path or complete hunk is removable |
| `GQD-0067` | `GQ1-CHUNK-0174` | `RETAIN` | Merged-wall interfaces, geometry classification, message delivery, and MIDI enumeration are branch-added Android owners. Remaining inherited renderer observations, warning callback installation, and game-data inputs are compact natural sites; further wrappers remove no material path or complete hunk and obscure ownership |
| `GQD-0068` | `GQ1-CHUNK-0175` | `NO_INHERITED_EFFECT` | MIDI interface, seek timeline, transfer policy, preview/transfer implementations, JNI/build owners, and focused tests are branch-added. No assigned symbol requires a base-existing D1/D2 hook or registration, for 0 attributable inherited paths, hunks, and lines |
| `GQD-0069` | `GQ1-CHUNK-0176` | `RETAIN` | Branch-added auto-network owners already centralize paired join/host policy. Remaining inherited menu, socket, mission, player-selection, and Netgame sites own local lifecycle/state; callbacks retain those observations and enlarge the interface. Existing calls and registrations are the narrow boundary, with no material path or complete hunk removable |
| `GQD-0070` | `GQ1-CHUNK-0177` | `RETAIN` | Branch-added MSAA and shader-runtime owners already centralize paired graphics policy. Remaining inherited OGL sites own frame nesting, context lifecycle, compile/link calls, and state binding; another facade retains those observations and enlarges the state contract. Existing calls and registrations are the natural boundary |
| `GQD-0071` | `GQ1-CHUNK-0178` | `NO_INHERITED_EFFECT` | Overlay/decoder owners, registrations, direct consumers, and PCM music implementation are branch-added. Base-existing paired song sites reach decoding only through that implementation and require no assigned symbol or hook. Removing or wrapping the scope eliminates 0 attributable inherited paths, hunks, and lines |
| `GQD-0072` | `GQ1-CHUNK-0179` | `RETAIN` | Branch-added helpers already centralize paired text reconstruction and durable file replacement. Remaining inherited play-save sites own game-specific layouts, offsets, values, and sequencing; another facade retains those calls and enlarges the format contract. Existing calls and registrations are the narrow boundary |
| `GQD-0073` | `GQ1-CHUNK-0180` | `RETAIN` | Branch-added owners centralize bounded wall-record validation, paired console capture, and indicator arithmetic. Remaining inherited sites supply one format read, console emission, or render-policy call and own local state; a larger facade retains those observations. Current calls and registrations are the narrow boundary |
| `GQD-0074` | `GQ1-CHUNK-0181` | `RETAIN` | Branch-added owners centralize paired font transformation, graphics mutation, inset policy, and config dispatch. Remaining inherited font, menu, gauge, and renderer sites own glyph state, option UI, HUD geometry, and frame consumption; another facade retains those observations. Existing calls and registrations are natural |
...
|---|---|---|---|---|---|
| `GQF-0001` | `FIXED` | P1/high | pr-hygiene | Root `dxx_matchmaking.db`, `.db-wal`, `.db-shm` | Audited runtime SQLite state was removed, root-anchored ignores prevent recurrence, and the policy regression proves no maintained fixture is masked under `GQR-0005` |
| `GQF-0002` | `FIXED` | P1/high | pr-hygiene | `game_data_to_copy_to_emulator/debuglog_20260520_211753.txt` | The audited 18.6 MB runtime debug log was removed and a narrow dated-debug-log recurrence rule is enforced under `GQR-0005` |
| `GQF-0003` | `FIXED` | P2/high | pr-hygiene | `android/test_output.txt`, `android/build_output.txt` | Both stale transcripts were removed and exact root-anchored Android transcript rules are enforced under `GQR-0005` |
| `GQF-0004` | `FIXED` | P3/high | pr-hygiene | Root `.tmp`, `test.txt`, `d1_d2_ogl_diff.txt` | All three audited scratch artifacts were removed with exact root-anchored recurrence rules while maintained fixture visibility remains tested under `GQR-0005` |
...
| `GQF-0051` | `FIXED` | P1/high | security/resource-exhaustion/memory-budget | STi2 method-15 decode scratch | One 128 MiB live-allocation budget now covers retained archives, output, decoder/list state, block and transform workspaces through direct, matching, SIT5 and nested paths. Exact-limit, one-over, allocation-failure, real-media and all-ABI validation passed under `GQR-0038` |
| `GQF-0052` | `FIXED` | P1/high | stability/JNI-boundary/resource-exhaustion | Assigned JNI array/string acquisitions and MIDI byte-array creation | Eight assigned JNI owners now check every fallible acquisition/allocation, preserve the first pending exception, release only acquired resources and stop before further JNI work. Nine contracts and all Android ABIs passed under `GQR-0039`; runtime fault injection remains unavailable |
...
| `GQF-0055` | `FIXED` | P1/high | security/protocol-authentication/replay | Reconnect request route proof across starting/waiting/migration | Starting, waiting and playing paths now stage route/proxy/context proof, commit the replay counter only after successful proof, preserve it through migration and mutate no route/readiness state before acceptance. Paired D1/D2 matrices passed under `GQR-0042` |
| `GQF-0056` | `FIXED` | P1/high | security/protocol-binding/compatibility | D1/D2 reconnect signed transcript and generation token | Transcript v2 binds D1/D2 title, request/challenge role and a 128-bit CSPRNG generation nonce replicated through migration; exact version/size gates reject downgrade. Cross-title, restart, endian, migration and downgrade vectors passed under `GQR-0043` |
| `GQF-0057` | `FIXED` | P1/high | security/resource-exhaustion/network | Reconnect request verification before admission | A fixed IP-keyed admission policy now limits crypto to four attempts per source and sixteen globally per second, shares request/proof capacity, ignores port rotation and rejects before JNI/JCA. Load, fairness, expiry and lifecycle-reset validation passed under `GQR-0044` |
...
| `GQF-0061` | `FIXED` | P1/high | security/resource-exhaustion/storage | Complete CD extraction attempt budget | One attempt-owned output, entry, live-memory, cancellation and free-space budget now spans all CD tracks, ISO/HFS fallback, mapped STi2, direct HFS and nested SOW in CLI and Android orchestration. Exact/one-over, release, cancellation and cleanup validation passed under `GQR-0048` |
...
| `GQF-0076` | `FIXED` | P1/high | security/resource-exhaustion/memory-budget | Inno metadata LZMA peak-live memory | One 128 MiB live-allocation budget now covers stored/raw metadata, decoded output including growth copies, and every LZMA dictionary/probability allocation. Exact-limit, one-over, decoder-OOM, real-installer and all-ABI validation passed under `GQR-0063` |
...
| `GQF-0078` | `FIXED` | P1/high | security/undefined-behavior/parser-validation | Inno version admission arithmetic | Every weighted signed version calculation was replaced by overflow-free tuple admission and schema comparison. Supported endpoints, adjacent boundaries, malformed components, `INT_MAX`, real installers and all Android ABIs passed under `GQR-0065`; UBSan was unavailable in the configured MSVC build |
...
| `GQF-0083` | `FIXED` | P1/high | security/resource-exhaustion/performance | Inno aggregate repeated solid-chunk decoding | One archive-owned 2 GiB decompressed-prefix work budget now charges every buffered and streamed zlib/LZMA restart and fails closed when exhausted; cancellation propagates mid-decode. Exact/one-over, 4,096-alias, unique-chunk, reset, real-installer and all-ABI validation passed under `GQR-0070` |
...
| `GQF-0100` | `FIXED` | P1/high | security/resource-exhaustion/storage/input-validation | ZIP early-marker prompt bypass | Raw/coherent local markers no longer lift the limit: generic one-shot streams are hard-capped at 16 MiB plus one probe byte and complete EOCD/central/local validation remains mandatory before exposure. Forged-header, exact/one-over and cleanup tests passed under `GQR-0087`; larger one-shot ZIPs are intentionally unsupported pending a prevalidated seekable path |
...
| `GQF-0103` | `FIXED` | P1/high | security/resource-exhaustion/storage/compression | Setup direct ZIP extraction budget | Direct Setup ZIP imports now receive one caller-owned cumulative `ExtractionBudget`, register every traversed entry, charge selected output while writing, seed nested SOW extraction with the same counters, propagate cancellation and clean failed staging. Focused exact/one-over JVM validation passed under `GQR-0090` |
| `GQF-0104` | `FIXED` | P1/high | security/resource-exhaustion/memory-budget | Kotlin bounded byte-array peak memory | Exact-size reads allocate one result array and unknown-size segmented reads account staging plus final overlap under 128 MiB; OOM cleanup and focused boundaries passed under `GQR-0091` |
...
| `GQF-0108` | `FIXED` | P1/high | security/resource-exhaustion/compression | RAR policy rejection fallback | Host fallback is now limited to typed capability/linkage absence; policy, limit, cancellation, integrity and unknown failures remain terminal and preserve identity under `GQR-0095` |
| `GQF-0109` | `FIXED` | P1/high | security/resource-exhaustion/memory/work | RAR catalog projection admission | Enumeration incrementally charges every native item, path/prefix work, retained entry, declared expansion and 128 MiB live memory before retention under `GQR-0096` |
...
| `GQF-0118` | `FIXED` | P1/high | security/resource-exhaustion/catalog-budget | Nested mission music container catalogs | One attempt-owned work, retained-record and retained-memory budget now spans outer archives, DXA and nested HOG catalogs, cancellation and final retention under `GQR-0105` |
| `GQF-0119` | `FIXED` | P1/high | security/resource-exhaustion/storage/compression | Streaming descriptor nested music budget | Streaming descriptors resolve exact sizes before retention, unusable compressed metadata fails ratio admission, nested helpers share one expansion budget and MIDI is capped at 64 MiB under `GQR-0106` |
...
| `GQF-0127` | `FIXED` | P1/high | security/resource-exhaustion/process-lifetime | Bounded extractor descendant ownership | POSIX attempt-owned process groups and gated Windows kill-on-close jobs tear down every descendant after success, failure, timeout and cancellation while preserving unrelated sentinels under `GQR-0114` |
| `GQF-0128` | `FIXED` | P1/high | security/filesystem-safety/resource-accounting | Extractor link and special-file outputs | Descriptor/handle-based no-follow traversal rejects links, reparse points, multiple links, special/sparse/cross-volume output and races, then charges max logical/allocated bytes before publication under `GQR-0115` |
...
| `GQ1-CHUNK-0168` | `CLEAN/NO_INHERITED_EFFECT` | Exact earlier public-surface review was clean; later frozen budget, cancellation, cache namespace, projection, and exception changes introduce no new root |
| `GQ1-CHUNK-0169` | `DUPLICATE/HISTORICAL-CLOSED/DEFER` | Open `BR-0029`; archived `BR-0081`, `BR-0065` repairs remain intact; minimization confirms deferred `GQD-0010` |
| `GQ1-CHUNK-0170` | `EXTENDS/HISTORICAL-CLOSED/RETAIN` | Open `GQF-0007`, `GQF-0020`, `GQF-0021`; accepted `GQF-0006` repair and archived bind-cache repair remain intact |
| `GQ1-CHUNK-0171` | `DUPLICATE/EXTENDS/RETAIN` | Open `BR-0208`, `BR-0332`, `BR-0259`, `BR-0229`, `BR-0396`; no new root |
| `GQ1-CHUNK-0172` | `DUPLICATE/EXTENDS/RETAIN` | Open `BR-0246`, `BR-0247`, `BR-0222`, `BR-0219`, `BR-0361`; no new root |
| `GQ1-CHUNK-0173` | `DUPLICATE/EXTENDS/RETAIN` | Open `BR-0236`, `BR-0029`; combined HUD section write narrows one path but does not close grouped publication |
| `GQ1-CHUNK-0174` | `DUPLICATE/EXTENDS/HISTORICAL-CLOSED/RETAIN` | Open `BR-0238`, `BR-0239`, `BR-0394`; catalog refactor closes earlier mixed-extension and 64-entry roots |
| `GQ1-CHUNK-0175` | `DUPLICATE/HISTORICAL-CLOSED/NO_INHERITED_EFFECT` | Open `BR-0016`, `BR-0276`; archived `BR-0028`, `BR-0030`, `BR-0319` repairs remain intact |
| `GQ1-CHUNK-0176` | `DUPLICATE/EXTENDS/RETAIN` | Open `BR-0257`, `GQF-0023`; tuple preflight narrows mutation but does not consume failed one-shot requests |
| `GQ1-CHUNK-0177` | `DUPLICATE/EXTENDS/RETAIN` | Open `BR-0251`, `BR-0647`; new diagnostics provide evidence but do not close recovery or smoke-oracle roots |
| `GQ1-CHUNK-0178` | `DUPLICATE/EXTENDS/HISTORICAL-CLOSED/NO_INHERITED_EFFECT` | Open `GQF-0058`, `BR-0286`; archived multichannel publication owner `BR-0252` remains closed |
| `GQ1-CHUNK-0179` | `DUPLICATE/EXTENDS/RETAIN` | Open `BR-0029`, `BR-0236`; C linkage repair does not change concurrency or grouped-publication roots |
| `GQ1-CHUNK-0180` | `DUPLICATE/EXTENDS/HISTORICAL-CLOSED/RETAIN` | Open `BR-0243`; archived wall-record bound owner `BR-0389` remains closed; indicator math is clean |
| `GQ1-CHUNK-0181` | `DUPLICATE/EXTENDS/HISTORICAL-CLOSED/RETAIN` | Open `BR-0029`, `BR-0236`, `GQF-0141`, `GQF-0142`; archived `BR-0199`, `BR-0200` remain closed; font scaling is clean |
...
| `GQC-0181` | `CLEAN` | `GQ1-CHUNK-0168` route-planner facade tail and public APIs | `GQD-0061` records no inherited effect. Declarations, projection bounds, cancellation and work budgets, exception containment, linkage, packing, callers, and focused tests were reconciled with historical route findings; no new owner survived |
| `GQC-0182` | `ISSUES` | `GQ1-CHUNK-0169` shared songs and startup resume | `GQD-0062` confirms the existing below-payoff songs deferral. Live thread-ownership evidence duplicates `BR-0029`; bounded playlist JSON and strict encoding repairs for archived `BR-0081` and `BR-0065` remain intact |
| `GQC-0183` | `ISSUES` | `GQ1-CHUNK-0170` Android OGL texture and viewport owners | `GQD-0063` retains the already-shared state-bearing boundary. Evidence extends open `GQF-0007`, `GQF-0020`, and `GQF-0021`; accepted filename-capacity and archived bind-cache repairs remain intact |
| `GQC-0184` | `ISSUES` | `GQ1-CHUNK-0171` rewind wrapper, RGBA helper, and route cache | `GQD-0064` retains the branch-added shared boundary. Evidence extends `BR-0208`, `BR-0332`, `BR-0259`, `BR-0229`, and the separate test-registration gap `BR-0396`; no new root |
| `GQC-0185` | `ISSUES` | `GQ1-CHUNK-0172` input-demo result, RNG-mode, and RNG trace | `GQD-0065` retains the already-shared observation boundary. Evidence extends `BR-0246`, `BR-0247`, `BR-0222`, `BR-0219`, and related producer root `BR-0361`; no new root |
| `GQC-0186` | `ISSUES` | `GQ1-CHUNK-0173` Android playsave preferences and layout | `GQD-0066` retains the shared format boundary. Combined HUD section writes improve one local transaction, while grouped multi-file/pilot publication remains `BR-0236` and concurrent launcher/engine access remains `BR-0029` |
| `GQC-0187` | `ISSUES` | `GQ1-CHUNK-0174` merged-wall APIs, geometry classifier, warning stub, and MIDI enumeration | `GQD-0067` retains the branch-added ownership boundary. Evidence extends `BR-0238`, `BR-0239`, and `BR-0394`; the newer mixed bounded HOG catalog keeps earlier truncation and extension-suppression defects closed |
| `GQC-0188` | `ISSUES` | `GQ1-CHUNK-0175` MIDI interface, seek timeline, and save-transfer policy | `GQD-0068` records zero inherited effect. Evidence extends `BR-0016` and `BR-0276`; synchronized initialization and sample-frame reconstruction keep archived `BR-0028`, `BR-0030`, and `BR-0319` closed |
| `GQC-0189` | `ISSUES` | `GQ1-CHUNK-0176` auto-network and shared UDP host/join policy | `GQD-0069` retains the shared network boundary. Evidence extends stale request owner `BR-0257` and bounded-wire-field investigation owner `GQF-0023`; tuple validation introduces no new root |
| `GQC-0190` | `ISSUES` | `GQ1-CHUNK-0177` Android MSAA and shader-runtime owners | `GQD-0070` retains the shared graphics boundary. Production diagnostics extend renderer recovery owner `BR-0251` and supply, but do not enforce, the effective-FBO oracle required by `BR-0647`; no new root |
| `GQC-0191` | `ISSUES` | `GQ1-CHUNK-0178` overlay ring and unified PCM decoders | `GQD-0071` records zero inherited effect. Complete decode allocation and blocking work extend `GQF-0058`; stale cross-run overlay use extends consumer owner `BR-0286`; result validation keeps archived `BR-0252` closed |
| `GQC-0192` | `ISSUES` | `GQ1-CHUNK-0179` play-save text and transaction helpers | `GQD-0072` retains the shared format boundary. Independent read-modify-replace operations extend concurrent ownership root `BR-0029` and incomplete grouped-publication root `BR-0236`; no new root |
| `GQC-0193` | `ISSUES` | `GQ1-CHUNK-0180` classic-demo wall validator, console ring, and cooperative indicator math | `GQD-0073` retains the shared boundary. Console continuation loss extends `BR-0243`; the new validator keeps archived `BR-0389` closed and the indicator math is clean |
| `GQC-0194` | `ISSUES` | `GQ1-CHUNK-0181` Android font scaling and graphics options | `GQD-0074` retains the shared graphics boundary. Evidence extends cross-thread owner `BR-0029`, grouped pilot persistence `BR-0236`, and transaction dependency roots `GQF-0141`/`GQF-0142`; archived `BR-0199`/`BR-0200` remain closed |
...
| `GQR-0005` | `DONE` | `GQF-0001` through `GQF-0004` | Remove tracked runtime/scratch artifacts and establish narrow recurrence policy | Nine audited targets totaling 19,150,156 bytes removed with narrow recurrence policy; durable plan `plan_gqr_0005_runtime_artifact_hygiene_20260813.md` |
...
| `GQR-0038` | `DONE` | `GQF-0051` | Enforce one peak-live-memory budget for STi2 method 15 | Exact/OOM, real-media and Android ABI validation passed; durable plan `plan_gqr_0038_sti2_method15_memory_budget_20260812.md` |
| `GQR-0039` | `DONE` | `GQF-0052` | Make assigned JNI acquisitions and allocations exception-safe | Eight JNI owners preserve first exceptions and transactional cleanup; durable plan `plan_gqr_0039_jni_exception_safety_20260813.md` |
...
| `GQR-0042` | `DONE` | `GQF-0055` | Require route proof before every reconnect state mutation | Paired state/route/proxy/NAT/replay/migration matrix and D1/D2 builds passed; durable plan `plan_gqr_0042_reconnect_route_proof_20260812.md` |
| `GQR-0043` | `DONE` | `GQF-0056` | Version and domain-separate reconnect transcripts/generations | Cross-title, role, restart, endian, migration and downgrade vectors passed; durable plan `plan_gqr_0043_reconnect_transcript_domain_separation_20260812.md` |
| `GQR-0044` | `DONE` | `GQF-0057` | Bound unauthenticated reconnect verification before JNI/JCA | Load/fairness/expiry/reset/source-rotation tests and paired/ABI builds passed; durable plan `plan_gqr_0044_reconnect_verification_admission_20260813.md` |
...
| `GQR-0048` | `DONE` | `GQF-0061` | Carry one budget through complete CD composition | Every track/fallback/nested operation now shares one attempt budget; native/Kotlin boundary and cleanup tests passed; durable plan `plan_gqr_0048_complete_cd_attempt_budget_20260812.md` |
...
| `GQR-0063` | `DONE` | `GQF-0076` | Enforce one peak-live-memory budget for Inno metadata decode | Exact/one-over/OOM, real-installer and Android ABI validation passed; durable plan `plan_gqr_0063_inno_metadata_peak_memory_20260812.md` |
...
| `GQR-0065` | `DONE` | `GQF-0078` | Make Inno version admission overflow-free | Tuple/parser/schema boundaries and real installers passed; UBSan unavailable; durable plan `plan_gqr_0065_inno_version_admission_20260813.md` |
...
| `GQR-0070` | `DONE` | `GQF-0083` | Bound aggregate Inno solid-chunk decode work | Exact/one-over, 4,096-alias, unique-chunk, reset and cancellation tests passed; durable plan `plan_gqr_0070_inno_decode_work_budget_20260813.md` |
...
| `GQR-0087` | `DONE` | `GQF-0100` | Require valid ZIP structure before lifting prompt limits | Forged markers cannot lift the 16 MiB one-shot cap; exact/one-over/cleanup tests passed; durable plan `plan_gqr_0087_zip_prompt_admission_20260813.md` |
...
| `GQR-0090` | `DONE` | `GQF-0103` | Apply one extraction budget to direct Setup ZIP import | Direct and nested Setup ZIP paths share cumulative counters and focused JVM tests passed; durable plan `plan_gqr_0090_setup_zip_extraction_budget_20260813.md` |
| `GQR-0091` | `DONE` | `GQF-0104` | Enforce peak-live-memory policy in Kotlin bounded reads | Exact and segmented peak accounting passed focused tests; durable plan `plan_gqr_0091_kotlin_bounded_read_peak_memory_20260813.md` |
...
| `GQR-0095` | `DONE` | `GQF-0108` | Prevent weaker fallback after RAR policy rejection | Typed capability-only fallback passed focused tests; durable plan `plan_gqr_0095_rar_terminal_policy_failure_20260813.md` |
| `GQR-0096` | `DONE` | `GQF-0109` | Bound RAR enumeration before materialization | Incremental entry/work/memory admission passed focused tests; durable plan `plan_gqr_0096_rar_enumeration_admission_20260813.md` |
...
| `GQR-0105` | `DONE` | `GQF-0118` | Carry one catalog budget through nested music containers | Shared outer/DXA/HOG catalog accounting passed focused tests; durable plan `plan_gqr_0105_nested_music_catalog_budget_20260813.md` |
| `GQR-0106` | `DONE` | `GQF-0119` | Preserve nested streaming size and expansion accounting | Descriptor, ratio, expansion and MIDI bounds passed focused tests; durable plan `plan_gqr_0106_streaming_music_source_budget_20260813.md` |
...
| `GQR-0112` | `DONE` | `GQF-0125` | Bind bounded extraction to an admitted Python runtime | Pinned or explicit path/SHA-256/version identity passed hostile-PATH tests; durable plan `plan_gqr_0112_bounded_python_runtime_20260813.md` |
...
| `GQR-0114` | `DONE` | `GQF-0127` | Supervise bounded extractor process trees | POSIX groups and Windows jobs passed terminal-path descendant tests; durable plan `plan_gqr_0114_bounded_process_tree_supervision_20260813.md` |
| `GQR-0115` | `DONE` | `GQF-0128` | Reject unsafe extractor output types | Physical no-follow validation and allocated-byte accounting passed Windows/WSL suites; durable plan `plan_gqr_0115_bounded_output_type_containment_20260813.md` |
...
### 2026-08-13: ranked remediations GQR-0038 through GQR-0090 completion

- Status: ten impact-rank-56 remediations are `DONE`; `GQF-0051`, `GQF-0055`, `GQF-0056`, `GQF-0057`, `GQF-0061`, `GQF-0076`, `GQF-0078`, `GQF-0083`, `GQF-0100`, and `GQF-0103` are `FIXED`
- Process: one fresh `gpt-5.6-sol` medium worker implemented each item in dependency-ordered waves, each wrote a durable tracked implementation plan, and the root writer independently reviewed scoped diffs and terminal evidence before updating this ledger
- Diff minimization: STi2, CD, Inno, and ZIP fixes remain wholly in branch-added Android/shared files. Reconnect policy remains in branch-added shared files; only symmetric protocol/state hooks and declarations touch paired inherited D1/D2 network files
- Combined validation: `run-windows-build.ps1 -Target both` passed. With JDK 21, `:app:externalNativeBuildDebug` passed D1/D2 for `arm64-v8a`, `armeabi-v7a`, and `x86_64`. Focused `ArchiveInputStreamsTest`, `ExtractionLimitsTest`, and `CueDataTrackExtractionTest` passed together. Registered D1 and D2 reconnect CTests passed. Each extraction worker also passed its recorded exact/one-over/OOM/cancellation/real-media or real-installer gates
- Compatibility decision: `GQR-0087` deliberately caps generic one-shot ZIP streams at 16 MiB plus one probe byte. Larger one-shot ZIP/SFX input is unsupported until a seekable or reopenable source can be structurally validated before large staging
- Validation limitations: UBSan was unavailable for `GQR-0065` in the configured MSVC build. Its direct tuple/parser/schema `INT_MAX` boundaries, real installers, and all Android ABI object builds passed instead
- Durable orchestration: `plan_next_ten_ranked_remediations_20260812.md` and the ten `plan_gqr_*` files named in the remediation rows preserve the item-specific scope, decisions, commands, and evidence

### 2026-08-13: ranked remediations GQR-0091 through GQR-0039 completion

- Status: impact ranks 23 through 32 are `DONE`; thirteen associated findings are `FIXED`, including the four artifact-hygiene owners closed together by `GQR-0005`
- Process: one fresh `gpt-5.6-sol` medium worker implemented each remediation in dependency-ordered waves with durable tracked plans; the root writer independently reviewed scoped diffs and terminal evidence
- Diff minimization: all ten remediations are confined to branch-added Android, helper, test, policy, and root ignore paths. Inherited D1/D2 source impact is zero
- Combined validation: `run-windows-build.ps1 -Target both` passed. With JDK 21, `:app:externalNativeBuildDebug` passed D1/D2 for arm64-v8a, armeabi-v7a and x86_64. The combined Kotlin suite for bounded reads, RAR policy/enumeration and mission-music catalog/staging passed. Item plans record Windows and WSL extractor-supervisor/output-containment suites, four real package regressions, artifact policy, JNI contracts and focused quality gates
- Artifact result: nine audited tracked runtime/scratch targets totaling 19,150,156 bytes and 57,330 text lines were removed; all remain recoverable from Git history. Root-anchored ignore rules and the repository-artifact policy test prevent recurrence without hiding maintained fixtures
- Platform limitations: Windows symbolic-link privilege was unavailable, so junction/reparse containment supplied that physical-escape oracle. Unprivileged WSL could not create a device node, while FIFO and Unix-socket rejection were exercised. No maintained runtime JNI fault injector exists, so JNI forced-failure sequencing is covered by source contracts plus real all-ABI compilation
- Durable orchestration: `plan_next_ten_ranked_remediations_20260813.md` and the ten item plans named in remediation rows preserve exact scope, decisions, commands and evidence
