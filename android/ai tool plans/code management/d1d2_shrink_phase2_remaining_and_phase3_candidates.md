...
- Status on current branch: the shared `sockaddr` / identity helper extraction is validated; follow-up trims have removed the `read_sync` per-player address dump, the generic `net_udp_listen` heartbeat, the success-path `[ANDROID]` sync or level-transition comments in `wait_for_sync`, `send_sync`, `request_poll`, and `level_sync`, the remaining success-path `MPDIAG(...)` chatter in `read_sync` and `send_sync`, the remaining success-path object-sync `MPDIAG(...)` chatter in `send_objects` and `read_object_packet`, the remaining success-path `verify_objects` `MPDIAG(...)` lines, the remaining success-path `add_player ... added as player` `MPDIAG(...)` line, a larger join-handshake bundle of success-path logs in `net_udp_new_player`, `net_udp_welcome_player`, `net_udp_add_player`, and `net_udp_process_request`, the remaining `game_connect` and `manual_join` success-path chatter, the remaining success-path `sync_token` and `game_info_token` `MPDIAG(...)` lines, the remaining packet-dispatch success-path chatter for `GAME_INFO_REQ`, `UPID_GAME_INFO`, and `UPID_REQUEST`, the remaining `request_poll`, `wait_for_requests`, and `level_sync` success-path summaries, the remaining Android auto-flow success-path chatter in `auto_join`, `auto_host`, and `net_udp_start_game`, the remaining direct-connection recovery success-path chatter in `net_udp_timeout_check`, `net_udp_process_p2p_ping`, `net_udp_process_p2p_pong`, `update_address_for_player`, and `reattemptDirect`, and the remaining routine join or game-info bookkeeping `con_printf(CON_DEBUG, ...)` lines for token-set success and player-address narration while keeping warning and failure paths intact; `net_udp_rebind_for_hosting`, `mpdiag_pkt_dump`, and the direct-reattempt helper set around `net_udp_send_p2p_reattempt_direct`, `net_udp_process_p2p_reattempt_direct`, `resetProxy`, and `reattemptDirect` are now shared with thin local wrappers in both games; the shared reattempt sender keeps the original packet layout by serializing `connect_to_player`, and the local wrappers still own the send, timer, and logger adapters; the previously suspected PDATA counter cleanup is not a logging-only slice on the live branch because it still feeds `Netgame.players[player].loss`; the next larger candidate is extracting the duplicated `net_udp_welcome_player` slot-selection and reconnect-reset helper cluster after `find_player_by_identity`, covering open-slot selection, oldest-disconnected replacement, reconnect address refresh, and Android connection-type reset while keeping the HUD, score, and object-sync side effects local; see `cleanup_net_udp_extract.md` for the tranche history and validation sequence
