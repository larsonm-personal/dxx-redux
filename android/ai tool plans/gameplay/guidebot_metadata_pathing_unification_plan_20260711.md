...
- [x] Phase 4 complete: separate canonical and live plans and bridge Guide-Bot to shared C++ output.
  ...
  - [ ] Remaining Phase 6 coverage: directed trigger crossing, blastable-wall semantics, and player-assisted boss, reactor, and exit completion. These remain passive player actions and must not alter classic Guide-Bot movement, steering, orientation, flare behavior, or RNG use.
  - [ ] Directed-crossing tranche in progress: represent an open-wall trigger with a safe activation point at the source segment center and an aim point in the child segment, then extend KCXF2 level 5 through red-key acquisition, a negative wait, ordinary player thrust across trigger 22, and automatic selection of the exit.
