...
- [ ] Phase 0 in progress: freeze corpus comparisons and characterize current live behavior.
  ...
  - Phase 6 now has player-assisted switch, hidden-door, directed-crossing, boss, reactor, blastable-wall, and terminal exit-pending fixtures.
...
- [x] Phase 4 complete: separate canonical and live plans and bridge Guide-Bot to shared C++ output.
  ...
  - [x] Added an owner-only, four-Hz passive observer for the current semantic waypoint. It performs no path search, ray cast, aiming, firing, or RNG work while a step remains pending; once the player's world-state change satisfies the shared predicate, it invalidates the high-level goal and lets the ordinary route lifecycle select the next waypoint.
  - [x] Hidden-door planning now uses the source segment center as its route and player activation position while retaining the wall face as the separate aim marker. This intentionally favors one simple planner representation over preserving incidental side-center travel distances.
  - [x] Final validation passed: D1 and D2 Windows builds and both native route snapshot suites pass; Android JVM tests and all three configured ABIs pass; the final APK passes both 42-step KCXF2 player-assisted fixtures without a second Guide-Bot command.
  - [x] Regenerated metadata retains the simpler hidden-door distance calculation. The full host corpus covered 1,244 levels across 109 passing mission archives, with one descriptor-less skip and zero failures, and no level changed route status. Every accepted JSON line change is a `distance`, `travel_distance`, or derived travel-time field.
  - [x] Phase 6 complete: blastable walls are explicit shared semantic steps with passive wall-state completion. The KCXF2 level 1 fixture proves Guide-Bot remains pending without player action, preserves classic path and RNG parity, and advances only after ordinary player-owned weapon damage.
  - [x] Directed-crossing tranche complete: open-wall triggers preserve established wall-target source ranking, then use the source segment center as the direct activation point and a distinct point across the source face as the aim marker. The fallback handles degenerate first face triangles by trying the quad's second triangle. The KCXF2 level 5 fixture proves Guide-Bot leaves trigger 22 blocked, crosses it only through ordinary player thrust, then selects the exit without entering it. Trigger 6 is advanced through the test harness after the negative Guide-Bot check because its reflective animated grate and player bob make a real shot nondeterministic.
  - [x] Boss, reactor, and exit-target tranche complete: a main-frame, owner-only passive monitor advances completed semantic goals even when the companion AI frame is absent. Boss and reactor fixtures use debug-only player-sourced engine damage, then prove the next shared objective is the exit. Exit plans preserve the selected side and expose a face-center aim marker plus a small inward activation pose; rendered end-level sequences satisfy the step without any Guide-Bot action.
  - [x] KCXF2 custom exits without a rendered end-level sequence immediately replace the level, so they do not expose an active `Endlevel_sequence` to automation. Live KCXF2 coverage therefore ends at `exit_pending`; native tests verify the selected side and inward activation pose, and ordinary engine level replacement remains the terminal completion behavior.
  ...
  - [x] Final Phase 6 corpus regeneration passed built-in Counterstrike and 109 mission archives with one expected descriptor-less skip and zero failures. All 1,274 reviewed levels retain their route status and problem text; 74 explicit blastable-wall steps were added and the reviewed fingerprint baseline was updated.
  - [x] Contained-key robot objectives now preserve carrier identity in the shared route step, tell the player to destroy the carrier, and draw the numbered automap objective at the robot's live position before following the dropped key. The 145 objectives across 99 levels use the same metadata and Guide-Bot semantics without changing classic movement, firing, or RNG behavior; the focused KCXF2 level 6 fixture and full corpus/build gates pass.
  - Final directed-crossing validation passes: full Windows D1/D2 builds, both route snapshot suites, Android JVM tests and all three ABIs, the 1,274-level checked corpus, strict base-campaign status checks, and the 50-step live KCXF2 route fixture. Full regeneration processed 1,244 levels across 109 archives with one descriptor-less skip and zero failures; route status, problem text, and semantic step identity remain unchanged while crossing guidance positions and derived travel fields were regenerated.
