...
- Phase 3 in progress
  ...
  - added `INPUT_DEMO_DEBUG_LOGGING_AVAILABLE` to `android/app/src/main/cpp/shared/input_demo_debug_logging.h`, so Android `NDEBUG` release-like native builds now get header-level no-op stubs while host builds and Android debug-like builds keep the existing runtime `-inputdemo-debug-log` switch
  - updated `d1/main/inferno.c` and `d2/main/inferno.c` so `-inputdemo-debug-log` is only advertised and scanned when that build actually compiled the replay debug helper in
  - kept the optional `Input demo replay path detail` and `Input demo replay path points` probes in `d2/main/aipath.c`, but reduced them to changed-state emissions keyed by the current object signature plus the emitted detail or point-sequence hash, so repeated identical path regeneration no longer logs the same snapshots every time
  - validated the compile-in path with `cmake --build buildd2 --target dxx-redux-d2-headless`, `cmake --build buildd1 --target dxx-redux-d1`, and the same D2 headless replay helper with and without `-ReplayDebugLog`, both ending at `RESULT: PASS`
  - extracted the shared replay probe activation helpers into `android/app/src/main/cpp/shared/input_demo_debug_logging.h/.cpp`, adding `input_demo_debug_record_probe_active()`, `input_demo_debug_activity_probe_active()`, and `input_demo_debug_replay_probe_active()` so the common `debug && record`, `debug && (record || replay)`, and `debug && replay` decisions no longer stay open-coded in the D2 gameplay files
  - updated the remaining duplicated D2 wrapper families in `d2/main/ai.c`, `aipath.c`, `controls.c`, `escort.c`, `object.c`, `physics.c`, and `render.c` to use the shared activation helpers and the existing shared frame or mode helpers instead of repeating the same `input_demo_debug_is_enabled()` plus recorder or replay checks locally
  - validated the shared-gate extraction with `android/tests/run_input_demo_replay.ps1 -DemoPath android/regression_demos/d2_descent2_level2_20260501_141150.dximdemo -Game d2 -Runner headless-console -ReplayDebugLog -HeadlessConsoleOutput 2`, which auto-rebuilt `d2` and completed with `RESULT: PASS`, then cross-checked the shared helper with `run-windows-build.ps1 -Target d1`, which completed with `Windows build complete for d1`
  - extracted shared frame-range helpers into `android/app/src/main/cpp/shared/input_demo_debug_logging.h/.cpp`, adding `input_demo_debug_frame_in_range()`, `input_demo_debug_activity_frame_in_range()`, and `input_demo_debug_replay_frame_in_range()` so the remaining probe window checks no longer stay open-coded in `d2/main/object.c` and `d2/main/render.c`
  - updated `d2/main/object.c` and `d2/main/render.c` to use the shared frame-range helpers and shared frame index directly, removing the last local lifecycle frame or mode wrappers for this slice and the duplicated render boundary window check
  - validated the frame-range extraction with `android/tests/run_input_demo_replay.ps1 -DemoPath android/regression_demos/d2_descent2_level2_20260501_141150.dximdemo -Game d2 -Runner headless-console -ReplayDebugLog -HeadlessConsoleOutput 2`, which auto-rebuilt `d2` and completed with `RESULT: PASS`, then cross-checked the shared helper with `run-windows-build.ps1 -Target d1`, and finally ran `android/run-code-quality.ps1 -Fix -Paths @('android/app/src/main/cpp/shared/input_demo_debug_logging.h','android/app/src/main/cpp/shared/input_demo_debug_logging.cpp','d2/main/object.c','d2/main/render.c')` before rerunning the same D2 helper to another `RESULT: PASS`
  - validated the Android release-like compile-out branch at the edited-object level with `ninja -C android/app/.cxx/RelWithDebInfo/4g2n2421/arm64-v8a ... inferno.c.o ... aipath.c.o ... input_demo_debug_logging.cpp.o`, which rebuilt the touched D1 and D2 release-like objects under `__ANDROID__` plus `NDEBUG`
  - validation note: the full Android `RelWithDebInfo` native build still fails earlier in unrelated existing `d1/3d/interp.c` and `d2/3d/interp.c` `nest_count` `Assert` errors under `NDEBUG`, so release-like verification for this slice has to stop at the edited object targets instead of the full shared libraries
