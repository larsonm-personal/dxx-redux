...
3. [completed] Use the first state/RNG/probe mismatch to choose the next D1-in-D2 semantic fix.
4. [completed] Rebuild and rerun the focused demo after each fix until the next divergence is understood or resolved.
5. [completed] Retarget the focused robot diagnostic to frame-29 object 11 and determine whether the exposed split is visibility/FVI, state transition, or turn integration.
6. [completed] Retarget the focused robot diagnostic to frame-32 object 16 after matching object 11, and determine whether the new fan-out is a shared AI-state transition or an over-broad D1 compatibility gate.
7. [completed] Retarget the focus window to object slots 128-159 and isolate the frame-45 bucket-4 robot-state divergence.
8. [completed] Trace and fix the final frame-2089 resource-drop mismatch so the Level 5 demo passes expected state checks under D1-in-D2.
9. [pending] Clean or make configurable the temporary diagnostics added during the Level 5 chase once the current green replay is safely preserved.
...
- Object 11 frame-29 is now understood: D2's `do_firing_stuff` could enter lock/fire logic through `Dist_to_last_fired_upon_player_pos` even with `player_visibility == 0`, while D1 only enters on `player_visibility >= 1`. D1-in-D2 now gates that D2-only shortcut off for D1 gameplay. This aligns object 11's frame-29 current/goal state, rotvel, orientation, and robot hash with native D1.
- The frame-32 object-16 mismatch was an extra D1-in-D2 follow-path-to-still random turn. Native D1 changes mode/hide/path state there and leaves turning to the later lower-state block. Removing the extra turn aligns object 16 through frame 35.
- The next gameplay mismatch is frame 45. Counts, weapons, fireballs, and focus slots 0-31 match; only robot/live-object hashes differ, isolated to object bucket 4 (slots 128-159). Retargeting the focus window to base 128 identified slot/object 151 as the first divergent object at frame 45; the detailed robot sample is now retargeted to object 151 to identify the field-level split.
- Object 151's hashed split is its rotational velocity. D1 starts a small turn at frame 45 while D1-in-D2 waits until frame 47, and the end-of-frame visibility state flips out of phase (`prev_vis` D1 0 vs D1-in-D2 1 at frame 45). The visibility/FVI probe is now retargeted from object 14 to object 151 to capture the exact ray result and dot gate around this frame.
- The object-151 FVI log exposed D2's later awareness upgrade in `compute_vis_and_vec`: raw visibility 1 is promoted to final visibility 2 when awareness is at least `PA_NEARBY_ROBOT_FIRED`. Native D1 leaves this raw value unchanged, so D1-in-D2 now gates that upgrade off while D1 gameplay compatibility is active.
- Gating the awareness upgrade is semantically correct but did not move the first mismatch. The object-151 split traces back to a latent orientation basis difference: the forward vector matches through frame 30, but after the same idle-turn RNG roll at frame 30, frame 31 forward vectors diverge. Added full orientation basis diagnostics (`rvec`, `uvec`, and `orient_hash`) to determine whether the hidden basis differs from restore or changes during the random-turn physics step.
- Full orientation diagnostics showed the basis matches through frame 30, then diverges after the frame-30 AIM_STILL turn. The root cause is the visibility vector source: native D1 computes `vis_vec_pos` from `calc_gun_point` whenever the robot is ready/fireable and near enough, while D2 only does this on `previous_visibility || !(obj_ref & 3)`. D1-in-D2 now uses D1's always-ready gun-point gate for this pre-visibility calculation, which moves the first selected mismatch from frame 45/object 151 to frame 56/bucket 0. The focus window is retargeted to slots 0-31 to isolate the new object.
- Later Level 5 fixes now verified against `d1_descent_level5_20260616_202713.dximdemo`: D1-in-D2 no longer creates D2-only water-hit vclips for D1 gameplay, which fixed the frame-298 object-174 fireball vclip mismatch (`VCLIP_WATER_HIT` versus the D1 explosion vclip). D1-in-D2 also now uses D1 auto-door close timing/state semantics, so frame 312 object 17 hits segment 51 side 4 and bounces like native D1 instead of flying through a D2-opened door.
- The frame-970 object-15 mismatch is fixed. Native D1 had object 15 in `AIM_CHASE_OBJECT` during a two-frame `SKIP_AI_COUNT` window after a claw contact, while D1-in-D2 kept it in `AIM_STILL` until the skip expired. D1-in-D2 now applies the D1 visible/aware normal-robot wake-up before returning from the skip path, aligning object 15 through the old frontier.
- Current Level 5 frontier after the skip wake-up fix is frame 1250. Native D1 has `live_object_count=153`, `fireball_object_count=3`, and `object_allocator_num_objects=153`; D1-in-D2 has `152`, `2`, and `152`. Focus slots 0 and 16 begin diverging at the same frame. Existing D2 logs around frames 1248-1251 show player weapon creation and an exploding robot/player contact in segment 128; native D1 lacks equivalent allocator/fireball detail, so the next diagnostic slice should either mirror the D2 allocator/fireball probes into D1 or retarget the fireball sample from object 174 to the frame-1250 created/missing fireball slot.
- The frame-1250 missing-fireball branch was D2's exploding robot/player collision early-return. Native D1 still creates the normal death explosion in that path, so D1-in-D2 now keeps the D1 collision behavior while D1 gameplay compatibility is active.
- The remaining final-state mismatch after the frame-1250 fix was shields at frame 2089. Native D1 spawned two shield boosts from robot 71; D1-in-D2 had the translated robot contents but D2's single-player high-shield/energy egg suppression consumed an extra RNG call and discarded the drop. D1-in-D2 now bypasses that D2-only suppression while D1 gameplay compatibility is active.
- D1-in-D2 replay result capture now uses D1's result convention for `robots_killed`, returning `num_robots_level` when D1 gameplay compatibility is active. This matches the native D1 replay-result payload for this D1 recording.
- Verified D2 build after the final Level 5 fixes with `.\run-windows-build.ps1 -Target d2`.
- Verified the Level 5 demo passes expected checks under D1-in-D2:
  `.\android\tests\run_input_demo_replay.ps1 -DemoPath C:\local\dxx-redux\android\regression_demos\d1_descent_level5_20260616_202713.dximdemo -Game d2 -D1InD2 -Mode accelerated -Runner fast -StateLogPath temp\d1_level5_202713_d1_in_d2_expected_check_state.jsonl -RngLogPath temp\d1_level5_202713_d1_in_d2_expected_check_rngtrace.jsonl -ReplayDebugLog -KeepSandbox -TimeoutSeconds 300`
- Passing D1-in-D2 final state now matches the native D1 expected payload: energy 52, shields 151, score 5200, segment 249, primary ammo 98, secondary ammo 3, robots_alive 89, robots_killed 106, powerups_remaining 48, and hostages 3.
