...
Status: implemented 2026-08-16; device interaction verification remains
...
- Record normalized active-song-list MIDI metadata in checked-in per-mod regression JSON such as `game_data/mission_files/Obsidian.json`
...
### 8. Add MIDI metadata to per-mod regression JSON

Add a normalized `music_tracks` array to each successful mission result generated by both the Android metadata analyzer and the headless host analyzer. Derive it from the active `BIMSongs` order after the mission SNG has been loaded so the regression output describes the tracks and slots the game will actually use, rather than every unused MIDI variant in an archive.

Each row should contain stable fields in this order:
...
Use `Title`, `Briefing`, `End level`, `End game`, `Credits`, and `Level music N` slot labels from the same five fixed indices already documented in the launcher. Keep a row when a song has no readable text metadata so changes in the active SNG and filename list remain regression-visible. Use empty strings or zero values consistently rather than omitting fields based on parser success.

The Android and host analyzers must call one shared native serializer over the same parsed metadata cache. Do not post-process checked-in JSON in PowerShell or Kotlin, and do not give the emulator and host regeneration paths separate MIDI rules. Keep output ordering deterministic and preserve the existing normalized pretty-printer.

`Obsidian.json` should therefore capture the HMP filenames selected by `obsidian.sng`, their exact MID metadata sources, inherited flags, inferred summaries, and bounded raw text events. The separately playable MID rows remain visible in launcher catalogs but are not duplicated into this runtime song-list regression section unless the SNG selects them.

## Implementation phases

### Phase 1: Native parser and inference contract

- [x] Add `midi_metadata.c` and `midi_metadata.h`
- [x] Parse bounded SMF structure and text meta events
- [x] Reuse HMP conversion and preserve valid UTF-8 output
- [x] Implement title, composer, display-name, and truncation policy
- [x] Add strict native fixtures for malformed input, output limits, encodings, and Obsidian-style metadata
...
- [x] Add the Kotlin metadata models and native parse bridge
- [x] Add metadata to installed-HOG enumeration during its existing track read
- [x] Add exact same-HOG HMP-to-MID inheritance
- [ ] Extend setup introspection with summary and inheritance fields for automation
- [x] Add model and JSON decode tests
...
- [x] Add the scrollable metadata printout dialog
- [x] Add `Metadata` to `MidiBytesPreviewDialog`
- [x] Show concise summary and inheritance note in the installed MIDI picker
...
- [x] Add pure same-container peer resolution to `MissionZipMusic.kt`
- [x] Add lazy parsing to `MissionZipMusicDialog`
- [x] Add `Metadata` in complete-catalog and selected-SNG modes
- [x] Preserve exact HMP playback while displaying inherited MID metadata
- [x] Add same-source, cross-source rejection, ambiguity, case, directory, and HMQ non-inheritance tests
...
- [x] Populate the shared embedded-MIDI name cache during the existing mission music-name load lifecycle
- [x] Preserve sidecar-name precedence
- [x] Add exact PhysFS HMP-to-MID fallback and bounded reads
- [x] Route list, current track, and overlay through the expanded resolver
...
- [x] Add shared active-song-list metadata serialization to JNI and headless mission outputs
- [ ] Add normalized `music_tracks` coverage to host/emulator parity tests
- [x] Regenerate `game_data/mission_files/Obsidian.json` through the standard generator after implementation
- [x] Inspect the Obsidian diff for active slot order, direct MID source attribution, inherited flags, summaries, and raw events
- [x] Run the new native metadata tests through CTest
- [x] Run focused Gradle tests for MIDI models and mission music resolution/display
...
- [x] Run D1/D2 Windows host builds, Android debug assembly, scoped code quality, and `git diff --check`
...
- A shared active-song-list metadata serializer used by both mission analyzers
...
- Host and emulator metadata can drift if music rows are appended in scripts. Serialize them from one shared native implementation instead
...
- Do not list every unused MIDI/HMP/HMQ archive member in per-mod regression JSON; record the active SNG song list and its metadata sources
