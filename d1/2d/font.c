...
#ifdef ANDROID
static int android_font_scale_active(void)
{
	return FNTScaleX < 0.99f || FNTScaleX > 1.01f || FNTScaleY < 0.99f || FNTScaleY > 1.01f;
}

static void android_render_scaled_font_char(int x, int y, unsigned char *fp,
                                            int raw_width, int draw_width,
                                            int draw_height, int underline,
                                            int masked)
{
	...
	if (raw_width < 1 || grd_curcanv->cv_font->ft_h < 1 || draw_width < 1 || draw_height < 1)
		return;

	bits_per_row = BITS_TO_BYTES(raw_width);
	gr_init_bitmap_alloc(&raw_bm, BM_LINEAR, 0, 0, raw_width, grd_curcanv->cv_font->ft_h, raw_width);
	for (r = 0; r < grd_curcanv->cv_font->ft_h; r++) {
		int underline_row = underline && (r == grd_curcanv->cv_font->ft_baseline + 2 || r == grd_curcanv->cv_font->ft_baseline + 3);
		for (i = 0; i < raw_width; i++) {
			int bit = fp[r * bits_per_row + (i >> 3)] & (0x80 >> (i & 7));
			raw_bm.bm_data[r * raw_width + i] = (bit || underline_row) ? fg : bg;
		}
	}

	draw_bm = &raw_bm;
	if (draw_width != raw_width || draw_height != grd_curcanv->cv_font->ft_h) {
		gr_init_bitmap_alloc(&scaled_bm, BM_LINEAR, 0, 0, draw_width, draw_height, draw_width);
...
static int android_internal_string_scaled_linear(int x, int y, const char *s, int masked)
{
	unsigned char *fp;
	const char *text_ptr, *next_row, *text_ptr1;
	int scaled_width, spacing, letter, raw_width, draw_width, draw_height, underline;
	int skip_lines = 0;
	int xx, yy = y;
	...
	while (next_row != NULL) {
		...
		while (*text_ptr) {
			...
			if (grd_curcanv->cv_font->ft_flags & FT_PROPORTIONAL)
				...
				fp = grd_curcanv->cv_font->ft_data + letter * BITS_TO_BYTES(raw_width) * grd_curcanv->cv_font->ft_h;

			android_render_scaled_font_char(xx, yy, fp, raw_width, draw_width, draw_height, underline, masked);
			xx += spacing;
			text_ptr++;
		}
		yy += skip_lines;
...
#ifdef ANDROID
	if (TYPE == BM_LINEAR && android_font_scale_active())
		return android_internal_string_scaled_linear(x, y, s, grd_curcanv->cv_font_bg_color == -1);
#endif
