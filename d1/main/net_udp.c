 ...
 */
int
net_udp_sync_poll( newmenu *menu, d_event *event, void *userdata )
{
	static fix64 t1 = 0;
	...
	if (Quitting)
		return -2;
#else
	(void)poll_count;  /* suppress unused warning when not using Android logging */
#endif
...
int net_udp_start_game(void)
{
	...
	if(net_udp_select_players())
	{
		...
		if (Netgame.gamemode == NETGAME_COOPERATIVE)
			COOPLOG("host StartNewLevel begin: game=d1 net_level=%d current_level=%d n_players=%d max=%d mission='%s'",
...
#ifdef __ANDROID__
	if (Netgame.gamemode == NETGAME_COOPERATIVE)
		COOPLOG("join StartNewLevel begin: game=d1 net_level=%d current_level=%d player_num=%d n_players=%d mission='%s'",
...
#ifdef __ANDROID__
/* android port: compare only IP + port, ignoring padding bytes in struct _sockaddr.
 * After socket rebind cycles during host migration, getaddrinfo vs recvfrom
 * can produce structs with different sin_zero padding, causing memcmp to
 * spuriously fail. */
static int sockaddr_equal(const struct _sockaddr *a, const struct _sockaddr *b)
{
