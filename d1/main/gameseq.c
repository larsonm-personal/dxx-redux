...
static int coop_start_too_close(vms_vector *pos, int assigned_count, fix min_dist)
{
	int i;
...
static int coop_find_fanout_start(int source, int assigned_count, vms_vector *pos, short *segnum)
{
	...
	for (scale_index = 0; scale_index < 2; scale_index++) {
		...
		for (offset_index = 0; offset_index < (int)(sizeof(offsets) / sizeof(offsets[0])); offset_index++) {
			const coop_start_offset *offset = &offsets[offset_index];
			vms_vector candidate = Player_init[source].pos;
			int candidate_seg;

			if (offset->r)
				vm_vec_scale_add2(&candidate, &Player_init[source].orient.rvec, scale * offset->r);
			if (offset->u)
				vm_vec_scale_add2(&candidate, &Player_init[source].orient.uvec, scale * offset->u);
			if (offset->f)
				vm_vec_scale_add2(&candidate, &Player_init[source].orient.fvec, scale * offset->f);
...
#ifdef __ANDROID__
	#define N_GLITZITEMS 19 /* base 9 + blank + up to 8 player kill rows + 1 header */
#else
