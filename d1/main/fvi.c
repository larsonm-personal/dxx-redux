...
static int check_line_to_face(vms_vector *newp,const vms_vector *p0,const vms_vector *p1,segment *seg,int side,int facenum,int nv,fix rad)
{
	...
	const int result = check_sphere_to_face(&checkp,seg,s,facenum,nv,rad,vertex_list);
	...
	if (flags & FQ_CHECK_OBJS)
		for (objnum=seg->objects;objnum!=-1;objnum=Objects[objnum].next)
			if (	!(Objects[objnum].flags & OF_SHOULD_BE_DEAD) &&
				...
				input_demo_log_fvi_weapon_robot_check(p0, p1, thisobjnum, objnum, fudged_rad, d); // android port
			...
			for (face=0;face<2;face++,bit<<=1) {
				...
				if (endmask & bit) {            //on the back of this face
					...
					if (face_hit_type) {            //through this wall/door
						...
						if ( (thisobjnum == Players[Player_num].objnum) && (cheats.ghostphysics || is_observer()) )	{
