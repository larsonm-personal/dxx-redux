	...
	if (Endlevel_sequence >= EL_FLYTHROUGH && Endlevel_sequence < EL_OUTSIDE)
		if ((explosion_wait2-=FrameTime) < 0) {
			...
			//create little explosion on wall
						...
						vm_vec_copy_scale(&tpnt,&ConsoleObject->orient.rvec,(d_rand_fx()-D_RAND_MAX/2)*100);
						vm_vec_scale_add2(&tpnt,&ConsoleObject->orient.uvec,(d_rand_fx()-D_RAND_MAX/2)*100);
...
void generate_starfield()
{
	...
	for (i=0;i<MAX_STARS;i++) {
				...
				stars[i].z = (d_rand_fx() - D_RAND_MAX/2) << 14;
