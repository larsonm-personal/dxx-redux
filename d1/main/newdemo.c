...
{
	const char *mission = input_demo_current_mission_id();

	if (!mission || !mission[0] || !strcmp(mission, "d1"))
		return "descent";
	return mission;
}
...
static void input_demo_fill_recorder_player_cfg(input_demo_recorder_settings *settings)
{
	...
	settings->player_cfg.primary_order_count = MAX_PRIMARY_WEAPONS + 2;
...
static void input_demo_build_quick_record_name(char *demo_name, unsigned int demo_name_size)
{
	...
	sprintf_s(base_name, SDL_arraysize(base_name), "d1_%s_level%d_%04d%02d%02d_%02d%02d%02d",
	          mission_slug,
