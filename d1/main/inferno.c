...
static int android_load_pilot_from_resume_save(const char *save_path)
{
	...
	debug_log(DLOG_GAME, "startup resume pilot prep: game=d1 path='%s' current_callsign='%s' use_players_dir=%d",
...
#ifdef __ANDROID__
	debug_log(DLOG_GAME, "startup args parsed: game=d1 Num_args=%d argc=%d", Num_args, argc);
	{
		int i;
		for (i = 0; i < Num_args; i++)
