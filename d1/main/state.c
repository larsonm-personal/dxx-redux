...
static void state_android_log_save_metadata(const char *filename, PHYSFS_file *fp)
{
	...
	if (have_meta) {
		...
		debug_log(DLOG_GAME,
			"restore save metadata: game=d1 file='%s' file_len=%lld digest_ok=%d fnv1a32=%08x meta_game=%u kind=%u wall=%llu callsign='%s' desc='%s' mission='%s' level=%d level_name='%s' level_seconds=%u total_seconds=%u thumb_format=%u thumb=%ux%u",
		...
		debug_log(DLOG_GAME,
			"restore save metadata: game=d1 file='%s' file_len=%lld digest_ok=%d fnv1a32=%08x have_android_meta=0",
...
// Render the current frame into cnv and write it out as a packed 6-bit
// RGB thumbnail for later preview. Matches the write side of
// state_read_thumbnail for STATE_VERSION >= STATE_THUMBNAIL_RGB_VERSION.
static void state_write_current_frame_thumbnail(PHYSFS_file *fp)
{
	grs_canvas *cnv = gr_create_canvas(THUMBNAIL_W, THUMBNAIL_H);
	grs_canvas *cnv_save;
	ubyte *rgb;
	int i;
...
#ifdef OGL
	{
		...
		// Y-flip and downsample to 6-bit Descent palette range
		k = THUMBNAIL_H;
		for (i = 0; i < THUMBNAIL_W * THUMBNAIL_H; i++) {
			int dst;
			if (!(j = i % THUMBNAIL_W))
...
//Restore GameTime
...
#ifdef __ANDROID__
	{
		...
		debug_log(DLOG_GAME,
			"restore save header: game=d1 file='%s' version=%d swap=%d desc='%s' mission='%s' level=%d gametime=%d current_callsign='%s' game_mode=%d",
...
#ifdef __ANDROID__
	debug_log(DLOG_GAME,
		"restore player state: game=d1 file='%s' callsign='%s' level=%d lives=%d objnum=%d n_players=%d game_mode=%d",
		filename, Players[Player_num].callsign, Players[Player_num].level,
		Players[Player_num].lives, Players[Player_num].objnum, N_players, Game_mode);
#endif
