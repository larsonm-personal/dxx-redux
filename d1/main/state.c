...
static int state_validate_runtime_state(PHYSFS_file *fp, int swap, int version)
{
	...
	if (!state_runtime_skip(fp, 2 * sizeof(int)) ||
	    ...
	    !state_runtime_read_s32(fp, swap, &laser_state.fusion_charge) ||
...
static int state_restore_all_sub_impl(char *filename, int *world_changed);
int state_restore_all_sub(char *filename)
{
	int world_changed = 0;
	android_restore_begin("d1", filename, Current_level_num,
...
#endif
{
	...
	if (!(Game_mode & GM_MULTI_COOP) && state_android_save_filename_is_coop(filename)) {
		debug_log(DLOG_GAME, "restore rejected coop save in single-player: game=d1 file='%s'", filename);
...
RetryObjectLoading:
	...
	if (Game_mode & GM_MULTI_COOP) {
		...
		COOPLOG("restore robot ownership payload: owned=%d expected=0",
		        restored_robot_owners);
	}
#endif
	...
	if (!ai_restore_state(fp, version, swap, rebirth)) {
		PHYSFS_close(fp);
		return 0;
	}
...
// Read Coop Info
	if (Game_mode & GM_MULTI_COOP)
	{
		...
		coop_normalize_restored_netgame_players("d1");
#endif
		for (i = 0; i < MAX_PLAYERS; i++)
		{
