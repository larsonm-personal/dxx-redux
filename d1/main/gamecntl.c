...
int HandleSystemKey(int key)
{
				...
				switch(choice)
				{
					case 0: // Abort Game
						...
						if (state_android_save_to_slot(ANDROID_SAVE_META_SLOT_AUTO_ABORT, "AUTO ABORT",
							...
							debug_log(DLOG_GAME, "autosave saved: D1 slot %d", ANDROID_SAVE_META_SLOT_AUTO_ABORT);
					#endif
						set_screen_mode(SCREEN_MENU);
						Current_level_num = 0;
	...
	switch (key)
	{
		...
		case KEY_ALTED+KEY_F3:
			 ...
			 * Now for all music
						debug_log(DLOG_GAME, "autosave abort requested: D1 level=%d", Current_level_num);
			 * ==============================================
			 */
...
static int input_demo_replay_apply_direct_commands(void)
{
	...
	for (direct_command_index = 0; direct_command_index != direct_command_count; ++direct_command_index) {
		...
		if (event.kind == INPUT_DEMO_REPLAY_DIRECT_COMMAND_CHANGE_DIFFICULTY &&
		    !difficulty_change_to(event.value0, DIFFICULTY_CHANGE_FROM_REPLAY))
