...
#ifdef __ANDROID__
	if (coop_recovery_active() && (uint32_t) GET_INTEL_INT(buf + 61) < coop_recovery_life(pnum)) return;
#endif
...
#ifdef __ANDROID__
	coop_recovery_set_omega(pnum, GET_INTEL_INT(buf + 57));
	coop_recovery_drop(pnum, (uint32_t) GET_INTEL_INT(buf + 61));
#endif
...
multi_send_player_explode(char type)
{
	...
	#ifdef __ANDROID__
	PUT_INTEL_INT(multibuf + 57, coop_recovery_omega(Player_num));
	PUT_INTEL_INT(multibuf + 61, coop_recovery_life(Player_num));
#else
