...
#ifdef __ANDROID__
	if (coop_recovery_active() && (uint32_t) GET_INTEL_INT(buf + 65) < coop_recovery_life(pnum)) return;
#endif
...
#ifdef __ANDROID__
	coop_recovery_set_omega(pnum, GET_INTEL_INT(buf + 61));
	coop_recovery_drop(pnum, (uint32_t) GET_INTEL_INT(buf + 57), (uint32_t) GET_INTEL_INT(buf + 65));
#endif
...
multi_send_player_explode(char type)
{
	...
	#ifdef __ANDROID__
	PUT_INTEL_INT(multibuf + 57, coop_recovery_player_revision(Player_num));
	PUT_INTEL_INT(multibuf + 61, coop_recovery_omega(Player_num));
	PUT_INTEL_INT(multibuf + 65, coop_recovery_life(Player_num));
#else
	PUT_INTEL_INT(multibuf + 57, 0);
	PUT_INTEL_INT(multibuf + 61, 0);
	PUT_INTEL_INT(multibuf + 65, 0);
#endif
...
void multi_send_ship_status_for_frame()
{
	...
	PUT_INTEL_INT(multibuf + 29, Players[Player_num].shields);
	PUT_INTEL_INT(multibuf + 33, Players[Player_num].score);
	PUT_INTEL_SHORT(multibuf + 37, Players[Player_num].primary_ammo[0]);
	PUT_INTEL_SHORT(multibuf + 39, Players[Player_num].primary_ammo[2]);
...
#ifdef __ANDROID__
	PUT_INTEL_INT(multibuf + 45, coop_recovery_player_revision(Player_num));
...
void multi_do_ship_status( const ubyte *buf, int authenticated_sender )
{
	...
	coop_recovery_set_player_revision(buf[1], (uint32_t) GET_INTEL_INT(buf + 45));
#endif
	if (is_observer())
	{
		...
		Players[buf[1]].shields = GET_INTEL_INT(buf + 29);
		Players[buf[1]].score = GET_INTEL_INT(buf + 33);
		Players[buf[1]].primary_ammo[0] = GET_INTEL_SHORT(buf + 37);
		Players[buf[1]].primary_ammo[2] = GET_INTEL_SHORT(buf + 39);
	...
	else if ((Game_mode & GM_MULTI_COOP) && buf[1] != Player_num)
	{
		...
		Players[pnum].secondary_weapon_flags = buf[19];
		Players[pnum].secondary_weapon = (sbyte)buf[20];
		Players[pnum].energy = GET_INTEL_INT(buf + 21);
		Players[pnum].shields = GET_INTEL_INT(buf + 29);
		Players[pnum].score = GET_INTEL_INT(buf + 33);
		Players[pnum].primary_ammo[0] = GET_INTEL_SHORT(buf + 37);
		Players[pnum].primary_ammo[2] = GET_INTEL_SHORT(buf + 39);
		Players[pnum].primary_ammo[3] = GET_INTEL_SHORT(buf + 41);
		Players[pnum].primary_ammo[4] = GET_INTEL_SHORT(buf + 43);
		int i;
...
void multi_send_restore_game(ubyte slot, uint id)
{
	...
	if ((Game_mode & GM_MULTI_COOP) && multi_i_am_master()) {
		...
		COOPLOG("multi_send_restore_game: game=d1 restore transfer failed, falling back to restore packet slot=%d id=%u file='%s'",
		        slot, id, filename);
	}
#endif
...
void multi_restore_game(ubyte slot, uint id)
{
...
#endif
	{
		...
		COOPLOG("multi_restore_game id mismatch: game=d1 file_id=%d request_id=%u file='%s'",
		        thisid, id, filename);
		if ((Game_mode & GM_MULTI_COOP) && !multi_i_am_master()) {
			COOPLOG("multi_restore_game id mismatch: game=d1 waiting for host restore transfer or resync");
#ifdef USE_UDP
	...
	if (!state_restore_all_sub(filename)) {
		COOPLOG("multi_restore_game rejected: game=d1 file='%s'", filename);
		coop_restore_status_failed();
		return;
	}
#else
