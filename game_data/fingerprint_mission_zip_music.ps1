...
#
# The output is game_data/music/Mission ZIP - <zip name>/chromaprint_info.jsonc
...
if (-not $SkipAcoustId) {
    $configPath = Join-Path $repoRoot "android/acoustid_config.jsonc"
    if (Test-Path $configPath) {
        try {
            $cfg = Read-JsoncFile $configPath
...
foreach ($zipFile in $zipFiles) {
    ...
    try {
        ...
        if (Test-Path $infoFile) {
            $existingInfo = Read-JsoncFile $infoFile
        ...
        $workInfoFile = Join-Path $workDir "chromaprint_info.jsonc"
...
if ($UpdateDatabase) {
    Write-Host "`nUpdating known_albums.jsonc from album sidecars"
