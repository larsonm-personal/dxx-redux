...
- mission metadata regeneration script choices:
  - Current default full regeneration: run the zero-parameter emulator wrapper from the repo root:
    ...
    ```
    This helper sets JDK 21 when available, builds the debug APK, installs it, runs the metadata-only mission ZIP batch, and writes normalized JSON next to each source archive. Use this when you want the current checked-in metadata regenerated and do not need custom options.
  ...
  - Fast host/no-emulator regeneration: run `.\android\helpers\regenerate_all_mission_metadata_host.ps1` from the repo root. It is a zero-parameter helper that builds or reuses the D1/D2 headless metadata analyzers, extracts mission archives locally, writes normalized JSON next to each source archive, and writes artifacts under `android\temp\mission_zip_host_metadata\...`. Use this for fast local metadata refreshes that do not need Android launcher/import/UI coverage.
