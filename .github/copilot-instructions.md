...
- before the first release (as in, currently), do *not* code for backwards compatibility within the launcher or android-specific code. do maintain compatibility in the d1/d2 dirs because it will help upstreaming. virtually every build will be a full apk rebuild with a data reset
  - treat Android save, config, and data formats as disposable: replace them directly without compatibility readers, migrations, branches, or tests unless explicitly requested
...
## debugging
- when making small debugging changes, *do not complain about a missing emulator and give up*. start the emulator as needed! `dxx-redux\android\Run-Emulator.ps1` will start an emulator for you
...
- mission metadata regeneration script choices:
  - Current default full regeneration: run the zero-parameter emulator wrapper from the repo root:
    ...
    ```
    This helper sets JDK 21 when available, builds the debug APK, installs it, runs the metadata-only mission ZIP batch, then runs configured extracted-CD missions through the host analyzer. It writes normalized checked-in JSON for both source types. Use this when you want the current checked-in metadata regenerated and do not need custom options.
  - Configurable emulator batch: use `android\helpers\run_mission_zip_batch.ps1` directly only when you need custom options such as `-MaxZips`, `-Pattern`, `-OutDir`, `-NoRegressionJson`, or a focused `-ZipDir`. This script has many parameters, uses or starts the emulator/device path, pushes archives into app-private storage, drives launcher automation, and writes detailed artifacts under the chosen output directory. Omit `-NoRegressionJson` when regenerating checked-in metadata.
  - Fast host/no-emulator regeneration: run `.\android\helpers\regenerate_all_mission_metadata_host.ps1` from the repo root. It is a zero-parameter helper that builds or reuses the D1/D2 headless metadata analyzers, analyzes configured extracted-CD missions, extracts mission archives locally, writes normalized checked-in JSON, and writes artifacts under `android\temp\mission_zip_host_metadata\...`. Use this for fast local metadata refreshes that do not need Android launcher/import/UI coverage.
